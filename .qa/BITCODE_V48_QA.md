...
- Observed: `pnpm -w --filter @bitcode/generic-doc-comments-doc-code build` fails with TS6059/TS6307 rootDir violations — the package extends the root tsconfig whose `paths` map `@bitcode/*` to package source trees, so `tsc -b` pulls `agent-generics/src` into the program.
...
### F8 — Prior product route route still live; auxillaries plumbing routes to it

- Severity: medium (legacy surface reachable post-launch; post-auth callback landed there until pinned to `/packs`)
- Observed: first successful wallet sign-in landed on `/packs` — `buildAuxillariesRoutePath` builds `/terminal?auxillary-open-to=…` via `AUXILLARY_OVERLAY_ROUTE_ROOT = '/terminal'` (`apps/uapi/app/auxillaries/components/auxillary-pane-meta.ts:18`), and the legacy cockpit page still renders.
- Canon: `/packs` functionality was split into `/packs`, `/reads`, and `/deposits`. A V48 gate should (a) verify each terminal capability was properly ported to the three routes, (b) retarget or remove `AUXILLARY_OVERLAY_ROUTE_ROOT` and remaining `/packs` links, and (c) remove the unused terminal page/code (relates to F4 — another surface the V47 feature-excess audit never classified).
- Interim fix: wallet sign-in post-auth destination pinned to `/packs` in `AuxillariesWalletConnectionPanel`; GitHub connect redirect retargeted to `/packs?auxillary-open-to=externals` in `tps/github/_callback-handler.ts`; all seven `auxillaries-contract.ts` repair/recovery routes retargeted from `/terminal?auxillary-open-to=…` to `/packs?auxillary-open-to=…` after "Add Email" landed on the legacy cockpit page with no email experience (the `AuxillariesProvider` reads the open-to param on any route, so panes open over `/packs`; the v40 browser-proof checker pins only `bitcode-browser-proof.ts` route states, untouched). Remaining `/packs` consumers (orbitals links, browser-proof paths, `AUXILLARY_OVERLAY_ROUTE_ROOT`/`buildAuxillariesRoutePath` itself) stay for the F8 gate.
...
- Trace: `/deposits` option synthesis runs `deposit-route-model.ts` → `buildDepositAssetPackOptions` (`packages/asset-packs-pipelines/domain/src/deposit-asset-pack-options.ts`) — three hardcoded `OPTION_BLUEPRINTS` with constant `measurementBias` (0.72/0.66/0.61) and FNV-hash pseudo-roots; the file still carries `approvedOptionsAdmittedBy: 'future-gate7-deposit-option-review'` scaffolding markers from an earlier version. `BITCODE_ASSET_PACK_REAL_INFERENCE` reaches only the pipeline-host QA routes (`/api/pipeline-host/asset-pack`), never the `/deposits` flow — the env flags change nothing for depositing. Admission/rejection rows are journal writes over the blueprints; the qa7 sha256 roots are the composer execution's deterministic evidence.
...
- Cause (throughput, related): the deposit agents (`deposit-codebase-comprehension`, `deposit-depository-search`, `deposit-inherent-regurgitation`, `deposit-input-comprehension`, `deposit-asset-pack-synthesis`) call `factoryPTRRAgent` directly and ALWAYS run the full PTRR × Failsafe × Thricified fan-out — they do NOT gate on the bounded profile (unlike the read agents' `shouldUseDiscoveryPtrr` deterministic/bounded fast-path). So even with `BITCODE_ASSET_PACK_REAL_INFERENCE_PROFILE=bounded` the deposit pipeline issues dozens of LLM calls and can exceed the 300s inline budget.
...
- Resolution (Garrett, 2026-06-26): both — remove inference profiles entirely (always-full hierarchy + real generation; F26-A) AND run the synthesis decoupled via the pipeline host (F26-B). Spec: F26 (`BITCODE_SPEC_V48_NOTES.md`).

### V48-Gate3-F26 — Non-configurable inference + decoupled host execution for SynthesizeAssetPacks

- Design (committed spec, F26): inference is non-configurable (no profiles/`shouldUsePtrr`/bounded/deterministic in-agent branches — always the full Pipeline→Phase→Agent→Step→Failsafe→Thricified→Generation hierarchy, always real inference; test determinism via boundary LLM mocks). And SynthesizeAssetPacks runs via the pipeline HOST, decoupled from the dispatching request: the route validates + dispatches, the host runs to completion while telemetry streams, the client reads the persisted synthesis on the completion event. Hosts: local in-process (dev) + Vercel Sandbox (prod).
...
- Observed (2026-07-05, commit `4fbc67e`, Preview builds): `Generating static pages`
  ...
  `/packs`, `/auxillaries`, all `/docs/[slug]`, and a prior product route, `/orbitals/*`,
...
**Status:** remediation R1–R7 on gate branch (classifier, scrub preserve
fullOptions, rehydrate-only settle, entitled rich artifact, history defense,
PR from request SHA, poll/UX). Operator scrub: service-role script
`scripts/scrub-unpaid-read-outputs.mjs` only (no HTTP).  
