...
exports.registerIntelligenceTools = registerIntelligenceTools;
 ...
 */
const effectivenessTrackingToolSchema = zod_1.z.object({
    operation: zod_1.z.enum(['measure', 'predict', 'learn', 'optimize'])
    ...
    beforeMetrics: zod_1.z.record(zod_1.z.any()).optional()
        .describe('Pre-execution metrics for comparison'),
    afterMetrics: zod_1.z.record(zod_1.z.any()).optional()
    ...
    proposedChanges: zod_1.z.array(zod_1.z.object({
        type: zod_1.z.string(),
    ...
    timeframe: zod_1.z.enum(['1d', '7d', '30d', '90d', 'all']).default('30d')
 ...
 */
const crossRepoLearningToolSchema = zod_1.z.object({
    operation: zod_1.z.enum(['extract', 'propagate', 'analyze', 'recommend'])
        .describe('Cross-repository learning operation'),
    // For pattern extraction
    sourceRepositories: zod_1.z.array(zod_1.z.object({
        owner: zod_1.z.string(),
        name: zod_1.z.string(),
        successMetrics: zod_1.z.record(zod_1.z.number()).optional()
    ...
    patterns: zod_1.z.array(zod_1.z.object({
        ...
        applicability: zod_1.z.array(zod_1.z.string())
    })).optional().describe('Patterns to propagate'),
    targetRepositories: zod_1.z.array(zod_1.z.object({
        owner: zod_1.z.string(),
        name: zod_1.z.string()
    ...
    repositoryContext: zod_1.z.object({
        owner: zod_1.z.string(),
        name: zod_1.z.string(),
        currentPatterns: zod_1.z.array(zod_1.z.string()).optional(),
    ...
    maxResults: zod_1.z.number().min(1).max(1000).default(50)
 ...
 */
const advancedWebResearchToolSchema = zod_1.z.object({
    query: zod_1.z.string().min(10)
        .describe('Research query with detailed context'),
    researchType: zod_1.z.enum([
    ...
    filters: zod_1.z.object({
        timeRange: zod_1.z.enum(['1d', '1w', '1m', '3m', '1y', 'all']).optional(),
        language: zod_1.z.string().optional(),
        minStars: zod_1.z.number().optional(),
        licensePlanning: zod_1.z.boolean().optional(),
        excludeDomains: zod_1.z.array(zod_1.z.string()).optional()
    }).optional().describe('Filters for research results'),
    synthesisType: zod_1.z.enum(['summary', 'comparison', 'analysis', 'recommendations'])
        .default('analysis').describe('Type of synthesis to perform'),
    maxResults: zod_1.z.number().min(5).max(100).default(25)
        .describe('Maximum results per provider'),
    includeUrlIntelligence: zod_1.z.boolean().default(true)
        .describe('Include URL credibility and content analysis'),
    contextAware: zod_1.z.boolean().default(true)
 ...
 */
const multimodalProcessingToolSchema = zod_1.z.object({
    attachments: zod_1.z.array(zod_1.z.object({
        type: zod_1.z.enum(['image', 'audio', 'video', 'document', 'figma', 'url']),
        content: zod_1.z.string(),
        metadata: zod_1.z.record(zod_1.z.any()).optional()
    })).min(1).describe('Attachments to process'),
    processingType: zod_1.z.enum([
    ...
    outputFormat: zod_1.z.enum(['structured', 'narrative', 'actionable', 'technical'])
        .default('structured').describe('Format for processing results'),
    crossModalSynthesis: zod_1.z.boolean().default(true)
        .describe('Perform cross-modal correlation and synthesis'),
    includeImplementationGuidance: zod_1.z.boolean().default(true)
        .describe('Include actionable implementation guidance'),
    contextRepository: zod_1.z.object({
        owner: zod_1.z.string(),
        name: zod_1.z.string()
    }).optional().describe('Repository context for processing'),
    qualityThreshold: zod_1.z.number().min(0).max(1).default(0.8)
 ...
 */
const enterpriseIntelligenceToolSchema = zod_1.z.object({
    ...
    scope: zod_1.z.enum(['team', 'department', 'organization', 'ecosystem'])
        .default('organization').describe('Scope of analysis'),
    timeHorizon: zod_1.z.enum(['current', '1m', '3m', '6m', '1y', 'trend'])
        .default('3m').describe('Time horizon for analysis'),
    analysisDepth: zod_1.z.enum(['executive', 'detailed', 'comprehensive'])
    ...
    includeBenchmarking: zod_1.z.boolean().default(true)
        .describe('Include industry benchmarking'),
    confidentialityLevel: zod_1.z.enum(['public', 'internal', 'confidential'])
 ...
 */
const marketplaceIntelligenceToolSchema = zod_1.z.object({
    operation: zod_1.z.enum([
    ...
    requirements: zod_1.z.object({
        description: zod_1.z.string(),
        technical_specs: zod_1.z.array(zod_1.z.string()).optional(),
        budget_range: zod_1.z.object({
            min: zod_1.z.number(),
            max: zod_1.z.number()
        }).optional(),
        timeline: zod_1.z.string().optional(),
        quality_requirements: zod_1.z.array(zod_1.z.string()).optional()
    ...
    filters: zod_1.z.object({
        ...
        priceRange: zod_1.z.object({ min: zod_1.z.number(), max: zod_1.z.number() }).optional(),
        ratings: zod_1.z.object({ min: zod_1.z.number(), max: zod_1.z.number() }).optional(),
        deliveryTime: zod_1.z.string().optional()
    }).optional().describe('Filters for marketplace search'),
    includeCompetitiveAnalysis: zod_1.z.boolean().default(true)
        .describe('Include competitive analysis'),
    riskTolerance: zod_1.z.enum(['low', 'medium', 'high']).default('medium')
        .describe('Risk tolerance for recommendations')
});
/**
 * Execute effectiveness tracking operations
 */
async function executeEffectivenessTracking(args, context) {
    ...
    try {
        switch (args.operation) {
            case 'measure':
                ...
                const effectiveness = calculateEffectiveness(args.beforeMetrics || {}, args.afterMetrics || pipeline.metrics || {}, pipeline);
            ...
            case 'predict':
                ...
                const prediction = await predictEffectiveness(args.repository, args.proposedChanges || [], context);
                return {
            ...
            case 'learn':
                ...
                const learningResults = await updateEffectivenessModels(args.outcomes, context);
                return {
            ...
            case 'optimize':
                ...
                const optimization = await optimizeForEffectiveness(args.targetMetrics || {}, args.constraints || [], context);
                return {
        ...
        logger_1.logger.error('Effectiveness tracking failed', { error, args });
        throw error;
    }
}
/**
 ...
 */
async function executeCrossRepoLearning(args, context) {
    try {
        switch (args.operation) {
            case 'extract':
                const patterns = await extractPatternsFromRepositories(args.sourceRepositories || [], context);
                return {
            ...
            case 'propagate':
                const propagation = await propagatePatterns(args.patterns || [], args.targetRepositories || [], context);
                return {
            ...
            case 'analyze':
                const analysis = await analyzeCrossRepoPatterns(args.analysisType || 'pattern_similarity', context);
                return {
            ...
            case 'recommend':
                const recommendations = await generateCrossRepoRecommendations(args.repositoryContext, context);
                return {
        ...
        logger_1.logger.error('Cross-repo learning failed', { error, args });
        throw error;
    }
}
/**
 ...
 */
function calculateEffectiveness(before, after, pipeline) {
    ...
    const overallScore = Object.values(improvements).reduce((sum, val) => sum + val, 0) / 4;
    return {
        overallScore,
...
async function predictEffectiveness(repository, changes, context) {
...
async function updateEffectivenessModels(outcomes, context) {
...
async function optimizeForEffectiveness(targets, constraints, context) {
...
async function extractPatternsFromRepositories(repos, context) {
...
async function propagatePatterns(patterns, targets, context) {
    return {
...
async function analyzeCrossRepoPatterns(analysisType, context) {
    return {
...
async function generateCrossRepoRecommendations(repoContext, context) {
    return {
        recommendations: [
...
Transforms any media type into actionable technical knowledge.`,
