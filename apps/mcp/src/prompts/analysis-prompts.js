...
exports.registerAnalysisPrompts = registerAnalysisPrompts;
...
Perfect for security audits, penetration testing preparation, and compliance reviews.`,
    arguments: zod_1.z.object({
        ...
        auditScope: zod_1.z.enum(['web-application', 'api', 'infrastructure', 'full-stack']).default('full-stack')
            .describe('Scope of the security audit'),
        complianceStandards: zod_1.z.array(zod_1.z.enum(['OWASP', 'GDPR', 'HIPAA', 'SOX', 'PCI-DSS', 'ISO-27001']))
            .optional().default(['OWASP']).describe('Compliance standards to evaluate against'),
        criticalAssets: zod_1.z.array(zod_1.z.string()).optional().default([])
            .describe('Critical assets or data that read special attention'),
        threatModel: zod_1.z.string().optional()
            .describe('Specific threat model or attack scenarios to consider'),
        environment: zod_1.z.enum(['development', 'staging', 'production', 'all']).default('production')
 ...
 */
const technicalDebtAnalysisPrompt = {
...
Ideal for technical debt audits, refactoring planning, and code quality initiatives.`,
    arguments: zod_1.z.object({
        ...
        analysisDepth: zod_1.z.enum(['surface', 'comprehensive', 'deep']).default('comprehensive')
...
Perfect for security audits, compliance reviews, and dependency management.`,
    arguments: zod_1.z.object({
        ...
        analysisScope: zod_1.z.enum(['direct', 'transitive', 'full']).default('full')
            .describe('Scope of dependency analysis'),
        securityFocus: zod_1.z.boolean().default(true)
            .describe('Include comprehensive security vulnerability analysis'),
        licensingFocus: zod_1.z.boolean().default(true)
            .describe('Include license compatibility and compliance analysis'),
        updateStrategy: zod_1.z.boolean().default(true)
            .describe('Include update recommendations and migration planning'),
        ecosystem: zod_1.z.array(zod_1.z.enum(['npm', 'pypi', 'maven', 'nuget', 'rubygems', 'cargo']))
 ...
 */
function registerAnalysisPrompts() {
    return [
        securityAuditPrompt,
