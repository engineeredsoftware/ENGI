...
export function DocsNextReadingCards({ page }: { page: BitcodeDocsPage }) {
  if (page.slug === 'product-workspace') {
    return (
      <div className="grid min-w-0 max-w-full gap-4 tablet:grid-cols-2">
        <Link
          href="/docs/product-actions"
          className="min-w-0 max-w-full rounded-none border border-emerald-300/14 bg-emerald-400/[0.06] p-4 transition hover:border-emerald-300/28 hover:bg-emerald-400/[0.1] phone:p-5"
        >
          <p className="text-[10px] uppercase tracking-[0.22em] text-emerald-200/70">Next</p>
          <p className="mt-2 break-words text-xl font-semibold text-white">Read every product write action</p>
        ...
        <Link
          ...
          className="min-w-0 max-w-full rounded-none border border-cyan-300/14 bg-cyan-400/[0.05] p-4 transition hover:border-cyan-300/28 hover:bg-cyan-400/[0.09] phone:p-5"
        >
          <p className="text-[10px] uppercase tracking-[0.22em] text-cyan-200/70">Audit</p>
          <p className="mt-2 break-words text-xl font-semibold text-white">Read proofs and readiness</p>
          <p className="mt-2 break-words text-sm leading-6 text-white/68">
