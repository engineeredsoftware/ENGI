 ...
 * V48 product language: Packs / Reads / Deposits; DataPacks; BTD volume & rights; BTC settlement.
 */
...
export const whatIsBitcodeSections = [
  {
    ...
    summary:
      'A DataPack is not a raw file upload, a tokenized repository, or a free-form AI answer. It is a measured, source-safe knowledge commodity that can satisfy a reviewed Need under auditable proof and settlement rules.',
    detail:
      'Bitcode starts with permitted source: code, files, designs, data, notes, commits, and metadata. Product routes synthesize that source into DataPack supply, score needs-fits, hold protected IP behind obfuscation and measurement surfaces, then prove settlement on a fully open-source, decentralized, and auditable ledger. Buyers pay with Bitcoin; BTD records volume and rights — not a second payment currency.',
    reason:
      'First-time readers should leave with the market object (the DataPack) and the safety rule (measurements visible; IP not) before learning any control surface.',
    points: [
      'Deposit lists DataPacks synthesized from source you approve — not unrestricted repo dumps.',
  ...
  },
  {
    id: 'product-map',
    eyebrow: 'Product map',
    title: '/deposits, /reads, and /exchange are one system — interfaces ride the same law',
    summary:
    ...
    detail:
      'No interface is a separate product. Every write must leave source-safe activity that /exchange can reread. Every delivery of protected contents depends on settlement finality and BTD rights transfer, not on which surface initiated the work.',
    reason:
    ...
    points: [
      '/exchange owns searchable activity and expandable proof, settlement, compensation, delivery, and repair detail.',
  ...
  },
  {
    ...
    detail:
      'Each product surface exposes one part of the chain. You write only when a bounded state change is intended, then reread /exchange (or the route’s own proof panel) before trusting fit, settlement, or delivery.',
    reason:
      'Learning is faster when every control is part of the value path rather than miscellaneous chrome.',
    steps: [
      'Start with DataPacks so the market object is clear.',
      'Open /deposits, /reads, and /exchange so the three product loops are familiar.',
  ...
  },
  {
    ...
    detail:
      'Testnet does not weaken identity, source safety, or delivery boundaries. Source-bearing DataPack contents stay withheld until testnet finality and rights transfer. Value-bearing mainnet settlement remains blocked until a promoted version authorizes it.',
    reason:
