 ...
 * Content-height only — no flex growth into residual.
 *
 * Laptop: ETH·BTC·SOL + $BTD/DataPack sit absolute above steps 3–4.
 * Phone: chips sit in a mid-row above cards 3–4 with ↓ arrows so they still
 * read as belonging to Pay rails / Mint when not overlaid on those cards.
 */
...
function RailChips() {
  const { rails } = BITCODE_PUBLIC_COPY.valueFlow;
  return (
...
function StepCard({
  ...
  step: (typeof BITCODE_PUBLIC_COPY.valueFlow.steps)[number];
  index: number;
}) {
...
export const MarketingLandingValueFlow = memo(function MarketingLandingValueFlow() {
  const { eyebrow, title, steps } = BITCODE_PUBLIC_COPY.valueFlow;
  ...
  return (
    ...
    >
      ...
      <div className="relative flex min-w-0 flex-col">
        <div className="relative min-w-0 pb-3 tablet:pb-5">
          <p className="text-[9px] font-semibold uppercase tracking-[0.2em] text-emerald-200/68">
            {eyebrow}
          </p>
          <h3 className="mt-1 max-w-full bg-gradient-to-r from-emerald-200 via-white to-cyan-100 bg-clip-text text-[0.95rem] font-semibold leading-snug text-transparent phone:text-[1.02rem] tablet:max-w-[55%] tablet:leading-none">
            {title}
          </h3>

          {/* Laptop: chips absolute over cols 3–4 (original geometry). */}
          <div
            className="pointer-events-none absolute bottom-0 right-0 top-0 z-[1] hidden w-[48%] min-w-0 items-end gap-0 pb-0.5 tablet:flex"
            aria-hidden="true"
          >
        ...
        {/*
          Phone: 1–2, then chip lanes with ↓ into 3–4 (chips not overlaid on cards).
          Tablet+: single row with between-step → and absolute chips.
        */}
        <div className="flex min-w-0 flex-col gap-1.5 tablet:hidden" aria-hidden={false}>
          <ol className="grid min-w-0 grid-cols-2 gap-1.5">
            {steps.slice(0, 2).map((step, index) => (
              <li key={step.label} className="flex min-w-0">
                <StepCard step={step} index={index} />
              </li>
            ))}
          </ol>
          <div className="grid min-w-0 grid-cols-2 gap-1.5" aria-hidden="true">
            <div className="flex flex-col items-center gap-0.5 pt-0.5">
              <RailChips />
              <span className="text-[10px] leading-none text-emerald-300/55">↓</span>
            </div>
            <div className="flex flex-col items-center gap-0.5 pt-0.5">
              <MintChips />
              <span className="text-[10px] leading-none text-emerald-300/55">↓</span>
        ...
        <ol className="hidden min-w-0 items-stretch gap-0 tablet:flex">
          {steps.map((step, index) => (
            <li key={step.label} className="flex min-w-0 flex-1 items-stretch">
              <StepCard step={step} index={index} />
              {index < steps.length - 1 ? (
                <span
                  className="mx-1 flex w-2.5 shrink-0 items-center justify-center self-center text-[9px] text-emerald-300/40"
                  aria-hidden="true"
                >
                  →
                </span>
