...
interface DeliveryTemplateTextProps {
  text: string;
...
export default function DeliveryTemplateText({
  text,
  templates,
  defaultNeed,
  onSelect,
      ...
      <span className="relative inline-block">
        <div className="relative inline-block group/template">
          ...
          {templates && templates.length > 0 && (
            <span className="absolute -bottom-1 right-[-0.4rem] pointer-events-auto z-20 overflow-y-hidden inline-block" style={{ contain: "layout paint" }}>
              <button onClick={(e) => { e.stopPropagation(); setShowTemplates(true); }} onMouseEnter={onMouseEnter} className="rounded-none group/toggle relative overflow-hidden flex items-center justify-between transition-[width] duration-300 ease-orbit-fluid max-w-fit text-purple-300 hover:text-purple-200">
        ...
        <AnimatePresence>
          {showTemplates && (
            <motion.div initial={{ opacity: 0, y: -5, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -5, scale: 0.95 }} transition={{ duration: 0.2 }} className="absolute left-0 top-full mt-1.5 z-50 min-w-[260px]">
              <div className="rounded-none border border-[#67feb7]/12 bg-[#030816]/98 backdrop-blur-sm shadow-lg" style={{ boxShadow: "0 4px 12px rgba(0,0,0,0.4), 0 0 0 1px rgba(103,254,183,0.06), inset 0 0 12px rgba(103,254,183,0.02)" }}>
                <div className="py-1 px-1">
                  {templates!.map((template) => (
                    <button data-testid={`template-item-${template.id}`} key={template.id} onClick={(e) => { e.stopPropagation(); onTemplateSelect(template.id); onSelect(template.text); setSelectedTemplate(template.name); setSelectedTemplateText(template.text); }} onMouseEnter={() => setHoveredTemplate(template.name)} onMouseLeave={() => setHoveredTemplate(null)} className="w-full px-2.5 py-1.5 text-left text-sm text-gray-400 hover:text-white rounded-none transition-all duration-150 group/template-item flex items-center justify-between relative">
                      <div className="absolute inset-0 rounded-none transition-all duration-150">
                        <div className={`absolute inset-0 rounded-none p-[1px] opacity-0 transition-opacity duration-150 ${hoveredTemplate === template.name || selectedTemplate === template.name ? "opacity-100" : ""}`} style={{ background: "linear-gradient(120deg, rgba(167,139,250,0.2), rgba(216,180,254,0.3))", WebkitMask: "linear-gradient(#fff 0 0) content-box, linear-gradient(#fff 0 0)", WebkitMaskComposite: "xor", maskComposite: "exclude" }} />
                        <div className={`absolute inset-0 rounded-none opacity-0 transition-opacity duration-150 ${hoveredTemplate === template.name || selectedTemplate === template.name ? "opacity-100" : ""}`} style={{ background: "linear-gradient(120deg, rgba(167,139,250,0.08), rgba(216,180,254,0.02))" }} />
                      </div>
