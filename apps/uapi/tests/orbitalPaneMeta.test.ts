...
describe('auxillary-pane-meta canonical routing', () => {
  ...
  it('builds overlay targets instead of standalone auxillaries pages', () => {
    expect(buildAuxillariesRoutePath('wallet')).toBe('/exchange?auxillary-open-to=wallet');
    expect(buildAuxillariesRoutePath('connects')).toBe('/exchange?auxillary-open-to=externals');
