...
describe('/api/templates/delivery route', () => {
  beforeEach(() => {
    jest.resetModules();
  });

  it('fails closed to an empty template list when no user session is present', async () => {
    const { GET } = await import('@/app/api/templates/delivery/route');

    const response = await GET(new Request('https://example.com/api/templates/delivery'));
