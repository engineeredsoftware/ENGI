...
const simpleSystemTextSearchExecute = jest.requireMock('@bitcode/generic-tools-simple-system-text-search').simpleSystemTextSearch.execute;
const webSearchExecute = jest.requireMock('@bitcode/generic-tools-web-search').search.execute;
const generateDigestMock = jest.requireMock('@bitcode/digest/run').generateDigest;
...
- Where can we simplify auth flows?`
    ...
    const tools = (0, tools_1.getBitcodeTools)();
    ...
    it('walks through the Yapper demo script end-to-end', async () => {
        ...
        const depiction = await runTool('depict_design_asset', {
        ...
        const designScaffold = await runTool('design_code', {
        ...
        const designUpdate = await runTool('design_code', {
        ...
        const plan = await runTool('code_design', {
        ...
        const codeChanges = await runTool('read_code_changes_from_vcs', {
        ...
        expect(listCommitsMock).toHaveBeenCalledWith(expect.any(Object), 'bitcode-labs', 'yapper', expect.objectContaining({ perPage: 2 }));
        // Step 5: Run repository search for context
        const search = await runTool('answer_codebase_query', { query: 'yapper' });
        expect(search.answer).toContain('apps/chatgpt/src/tools.ts:11');
        // Step 6: Run Web search
        const web = await runTool('answer_codeweb_query', { query: 'voice-first optimistic ui' });
        expect(web.answer).toContain('Voice-first best practices');
        // Step 7: Vercel status check
        const vercelStatus = await runTool('use_vercel_read_external_mcp', {
            request: 'list_deployments',
            payload: { projectId: 'prj_Yapper', teamId: 'team_bitcode' }
  });
        expect(vercelStatus.answer.deployments).toBeInstanceOf(Array);
        // Step 8: AWS health check
        const awsHealth = await runTool('use_aws_read_external_mcp', {
        ...
        const awsWrite = await runTool('use_aws_write_external_mcp', {
        ...
        const agentsUpdate = await runTool('improve_developing_behavior', {
