...
**Why it matters**  
- Keeps `.docs/PRODUCT.md` and `.docs/AGENTS.md` as the single source of truth for intent and collaboration.  
...
2. `BITCODE_LOG_STDOUT=0 pnpm --silent --filter @bitcode/external-apps-chatgpt start` to expose stdio MCP server without polluting STDIO.  
...
**Key guardrails**  
- Never skip updating `.docs/PRODUCT.md` / `.docs/AGENTS.md` when design or behaviour shifts.  
- Ask for confirmation before any `write_code_changes_to_vcs`, `use_vercel_write_external_mcp`, or `use_aws_write_external_mcp` call, then pass `confirmed: true` so the tool emits a target-bound Bitcode write-admission receipt for the GitHub/Vercel/AWS delivery mechanism.
