...
- **REQUIRED — full green CI before every commit.** Commits must always be fully green. Before committing (and before pushing), run the living required application CI surface mirrored by `.github/workflows/ci.yml` lint-build at minimum: `pnpm run build:eslint-plugin`, `pnpm -C apps/uapi run lint`, `pnpm -C apps/uapi exec tsc --noEmit`, `pnpm -C apps/uapi run build`, plus focused Jest/package tests for the change. Gate/version work also requires living active+draft quality (gate-quality / canon-quality). Version → `main` requires the promotion workflow green. Do not commit known red typecheck/lint/build. Partial “one test file” smoke is iteration only — not a commit bar. Human guide: `CONTRIBUTING.md` §8.1.
