...
| shippable/AssetPack support surfaces still named around retained output nouns | `StreamFactory.createShippablesStream`, `branchAssetPackRun`, `AssetPackPipelineToolSchema`, `PipelineNameValues = ['asset-pack']`, `apps/uapi/components/shippables/*`, `apps/uapi/stories/shippables/*`, and `asset-pack-scenarios.ts` | streams, branch execution, MCP, UI components, stories, and drymock scenarios now teach current Bitcode owners while retained route/storage fields stay compatibility-bound |
...
- active tests and Storybook carriers are proof-bearing import-path owners too: where canonical auxillary surfaces already exist, files such as `apps/uapi/tests/{orbitalsPaneTabs,orbitalsWorkspacePanels,orbitalPaneMeta}.test.*` and `apps/uapi/stories/{LoginPane,OnboardingPane,user/Auth,user/Onboarding}.stories.*` must import `apps/uapi/app/auxillaries/*` rather than teaching compatibility `orbitals` owners as current truth.
