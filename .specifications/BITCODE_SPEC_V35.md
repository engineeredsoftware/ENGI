...
6. generated V35 artifacts under `.proofs/` when produced.
...
- `LocalStagingTelemetryDocumentationRehearsal`: local/staging-testnet lane id, documentation discovery phase, telemetry event emission phase, dashboard/runbook lookup phase, docs QA phase, incident drill phase, source-safe proof-root review phase, redacted screenshot/log roots, evidence roots, and blocked value-bearing mainnet posture.
...
9. **Gate 9: Local Staging Telemetry Documentation Rehearsal** materializes `LocalStagingTelemetryDocumentationRehearsal` to rehearse local and staging-testnet telemetry/docs visibility, dashboard/runbook paths, docs QA, incident drills, source-safe proof roots, and blocked value-bearing mainnet posture.
...
The deterministic artifact `.proofs/v35/documentation-surface-catalog.json` carries `source-safe-documentation-surface-metadata` and proves twelve required documentation surface rows:
...
The deterministic artifact `.proofs/v35/telemetry-taxonomy-catalog.json` carries `source-safe-telemetry-taxonomy-metadata` and proves fourteen required event-family rows:
...
The deterministic artifact `.proofs/v35/public-docs-usage-guides.json` carries `source-safe-public-docs-metadata` and proves twelve public usage guide rows:
...
The deterministic artifact `.proofs/v35/operator-runbook-catalog.json` carries `source-safe-runbook-metadata` and proves fourteen dashboard, alert, runbook, incident, and escalation rows derived from the fourteen `TelemetryTaxonomyCatalog` event families:
...
The deterministic artifact `.proofs/v35/docs-qa-alignment-report.json` carries `source-safe-docs-qa-metadata` and proves ten documentation alignment rows:
...
## V35 TestnetRolloutReadinessGuide canon

Gate 7 materializes `TestnetRolloutReadinessGuide` as package-owned protocol truth.
The deterministic artifact `.proofs/v35/testnet-rollout-readiness-guide.json` carries `source-safe-rollout-guide-metadata` and proves ten rollout guide rows:
...
Every row names audience, purpose, lane ids, workflow stages, source roots, reproducible commands, source-safe guide examples, known blockers, rehearsal evidence, validation commands, allowed disclosure, forbidden disclosure, fail-closed result, source evidence, and deterministic guide root.
The guide distinguishes `local`, `staging-testnet`, `public-testnet`, `mainnet-ready-dry-run`, and `value-bearing-mainnet`.
The `value-bearing-mainnet` lane remains visible and explicitly blocked as `blocked_future_canon_required`; no V35 rollout guide may make value-bearing mainnet admissible.
Rollout guide payloads may carry lane ids, guide ids, route names, validation commands, proof roots, dashboard panel ids, runbook ids, redacted failure classes, public docs slugs, source-safe examples, known blocker names, and rehearsal evidence paths.
They must not carry secret values, provider tokens, wallet private material, raw protected prompts, protected source payloads, buyer repository private data, raw protected model responses, or unpaid AssetPack source.
The guide fails closed on missing guide ids, missing lane ids, missing source roots, missing reproducible commands, missing examples, missing known blockers, missing rehearsal evidence, missing validation commands, secret-shaped markers, protected source disclosure, wallet private material disclosure, raw protected prompts, unpaid AssetPack source disclosure, or any unblocked value-bearing mainnet posture.
The artifact is generated by `pnpm run generate:v35-testnet-rollout-readiness-guide`, checked by `pnpm run check:v35-testnet-rollout-readiness-guide`, closed by `pnpm run check:v35-gate7`, and exported by `@bitcode/protocol` through `buildTestnetRolloutReadinessGuide`.

## V35 TelemetryDocumentationInterfaceIntegration canon

Gate 8 materializes `TelemetryDocumentationInterfaceIntegration` as package-owned protocol truth.
The deterministic artifact `.proofs/v35/telemetry-documentation-interface-integration.json` carries `source-safe-interface-integration-metadata` and proves eight active interface rows:
...
Every row names integration surface, owner, docs links, event families, event ids, proof root fields, runbook links, redaction posture, correlation ids, source roots, route payload surfaces, validation commands, allowed payload fields, forbidden payload classes, fail-closed result, source evidence, and deterministic integration root.
Interface payloads may expose event ids, proof roots, docs links, runbook links, redaction posture, correlation ids, state labels, summary counts, and source-safety class when those fields help an operator or interface consumer move from a visible state to a documented repair path.
They must not carry secret values, provider tokens, wallet private material, protected source payloads, raw protected prompts, raw protected model responses with protected source, buyer repository private data, or unpaid AssetPack source.
Terminal, Auxillaries, API, MCP API, ChatGPT App, package README, internal docs, and public docs must consume package-owned documentation and telemetry contracts rather than route-local or interface-local copies.
The integration fails closed on missing interface rows, missing source roots, missing docs links, missing event ids, missing proof root fields, missing runbook links, missing redaction posture, secret-shaped markers, protected source disclosure, wallet private material disclosure, raw protected prompts, raw protected model responses with source, buyer repository private data, or unpaid AssetPack source disclosure.
The artifact is generated by `pnpm run generate:v35-telemetry-documentation-interface-integration`, checked by `pnpm run check:v35-telemetry-documentation-interface-integration`, closed by `pnpm run check:v35-gate8`, and exported by `@bitcode/protocol` through `buildTelemetryDocumentationInterfaceIntegration`.

## V35 LocalStagingTelemetryDocumentationRehearsal canon

Gate 9 materializes `LocalStagingTelemetryDocumentationRehearsal` as package-owned protocol truth.
The deterministic artifact `.proofs/v35/local-staging-telemetry-documentation-rehearsal.json` carries `source-safe-rehearsal-metadata` and proves six required rehearsal rows:
...
Each row binds lane id, documentation surface ids, interface ids, telemetry event families, event ids, proof root fields, dashboard panel ids, runbook links, docs QA ids, rollout guide ids, validation commands, evidence roots, redacted screenshot/log roots, incident classes, and source-safety posture.
Local and staging-testnet rehearsal rows must include documentation discovery, telemetry event emission, dashboard/runbook lookup, docs QA review, incident drill, and source-safe proof-root review.
Mainnet-ready dry-run review may inspect proof roots without value-bearing unlock; value-bearing mainnet remains visible and blocked.
Rehearsal payloads may expose lane ids, event ids, proof roots, docs links, runbook ids, dashboard ids, incident classes, redaction posture, source-safety class, and summary counts.
They must not carry secret values, provider tokens, wallet private material, protected source payloads, raw protected prompts, raw protected model responses with protected source, buyer repository private data, or unpaid AssetPack source.
The artifact is generated by `pnpm run generate:v35-local-staging-telemetry-documentation-rehearsal`, checked by `pnpm run check:v35-local-staging-telemetry-documentation-rehearsal`, closed by `pnpm run check:v35-gate9`, and exported by `@bitcode/protocol` through `buildLocalStagingTelemetryDocumentationRehearsal`.
...
- Current source-bearing implementation basis: protocol package helpers, `.proofs/` artifacts, workflows, V35 gate checkers, and deployment playbooks.
...
- proofArtifactPath: `.proofs/v35/telemetry-docs-inference-synthesis-proof.json`
...
- witnessArtifactPaths: `.proofs/v35/spec-family-report.json`, `.proofs/v35/canonical-input-report.json`
...
- generated-artifact and test bindings: V35 telemetry/documentation artifacts through Gate 10, including `.proofs/v35/documentation-telemetry-promotion-readiness-report.json`.
...
- proofArtifactPath: `.proofs/v35/telemetry-docs-prompt-completeness-proof.json`
...
- witnessArtifactPaths: `.proofs/v35/spec-family-report.json`
...
- proofArtifactPath: `.proofs/v35/telemetry-docs-static-code-analysis-proof.json`
...
- witnessArtifactPaths: `.proofs/v35/canonical-input-report.json`
...
- proofArtifactPath: `.proofs/v35/telemetry-docs-verification-decisions-proof.json`
...
- proofArtifactPath: `.proofs/v35/telemetry-docs-selection-materialization-proof.json`
...
- proofArtifactPath: `.proofs/v35/telemetry-docs-authorization-sensitive-flow-proof.json`
...
- proofArtifactPath: `.proofs/v35/telemetry-docs-settlement-source-to-shares-proof.json`
...
- proofArtifactPath: `.proofs/v35/telemetry-docs-disclosure-boundary-proof.json`
...
- proofArtifactPath: `.proofs/v35/telemetry-docs-proof-contract-proof.json`
...
The readiness report `.proofs/v35/documentation-telemetry-promotion-readiness-report.json` proves that all V35 telemetry/documentation artifacts are present, source-safe, parseable, wired into gate checks, wired into promotion checks, generated proof appendix support, and workflow validation.
...
Promotion rewrites `BITCODE_SPEC.txt` from `V34` to `V35`, generates `BITCODE_SPEC_V35_PROVEN.md`, records source-safe `.proofs/v35/*` evidence, and prepares active V35 / draft V36 posture.
...
Primary V35 types: `TelemetryTaxonomyCatalog`, `DocumentationSurfaceCatalog`, `DocsQaAlignmentReport`, `OperatorRunbookCatalog`, `TestnetRolloutReadinessGuide`, `TelemetryDocumentationInterfaceIntegration`, `LocalStagingTelemetryDocumentationRehearsal`, and `DocumentationTelemetryPromotionReadinessReport`.
...
| --- | --- | --- | --- | --- | --- | --- |
| Inference-synthesis | `.proofs/v35/telemetry-docs-inference-synthesis-proof.json` | runtime inference receipts | deployment-inference-runtime-receipted | run fixture | `.proofs/v35/spec-family-report.json` | V34 Reading plus V35 runtime receipts |
| Prompt-completeness | `.proofs/v35/telemetry-docs-prompt-completeness-proof.json` | prompt digest receipts | deployment-prompts-redacted | redaction check | `.proofs/v35/spec-family-report.json` | V34 prompt registry plus V35 lane receipts |
| Static-code-analysis | `.proofs/v35/telemetry-docs-static-code-analysis-proof.json` | route and import scans | deployment-routes-unversioned | route scan | `.proofs/v35/canonical-input-report.json` | source tree |
| Verification-decisions | `.proofs/v35/telemetry-docs-verification-decisions-proof.json` | lane decisions | deployment-approval-verified | denied fixture | `.proofs/v35/spec-family-report.json` | V35 lane and approval contracts |
| Selection-and-materialization | `.proofs/v35/telemetry-docs-selection-materialization-proof.json` | storage materialization | deployment-delivery-paid-only | storage fixture | `.proofs/v35/canonical-input-report.json` | V34 AssetPack plus V35 storage posture |
| Authorization-and-sensitive-flow | `.proofs/v35/telemetry-docs-authorization-sensitive-flow-proof.json` | secret and auth rows | deployment-secrets-not-serialized | secret scan | `.proofs/v35/spec-family-report.json` | V34 auth plus V35 rotation contracts |
| Settlement-source-to-shares | `.proofs/v35/telemetry-docs-settlement-source-to-shares-proof.json` | settlement observers | deployment-settlement-root-stable | finality fixture | `.proofs/v35/canonical-input-report.json` | V34 BTD settlement plus V35 observers |
| Disclosure-boundary | `.proofs/v35/telemetry-docs-disclosure-boundary-proof.json` | storage disclosure | deployment-preview-not-source | redaction scan | `.proofs/v35/spec-family-report.json` | V34 disclosure plus V35 storage |
| Proof-contract | `.proofs/v35/telemetry-docs-proof-contract-proof.json` | promotion proof | deployment-proof-replayable | promote | `.proofs/v35/spec-family-report.json` | V35 generated artifacts |
...
| --- | --- | --- | --- |
| `.proofs/v35/spec-family-report.json` | protocol | source-safe | `node scripts/check-bitcode-spec-family.mjs --version V35 --mode draft --current-target V34` |
| `.proofs/v35/canonical-input-report.json` | protocol | source-safe | `node scripts/check-bitcode-canonical-inputs.mjs --current-target V34` |
| `.proofs/v35/documentation-surface-catalog.json` | protocol | source-safe-documentation-surface-metadata | `pnpm run check:v35-gate2` |
| `.proofs/v35/telemetry-taxonomy-catalog.json` | protocol | source-safe-telemetry-taxonomy-metadata | `pnpm run check:v35-gate3` |
| `.proofs/v35/public-docs-usage-guides.json` | protocol | source-safe-public-docs-metadata | `pnpm run check:v35-gate4` |
| `.proofs/v35/operator-runbook-catalog.json` | protocol | source-safe-runbook-metadata | `pnpm run check:v35-gate5` |
| `.proofs/v35/docs-qa-alignment-report.json` | protocol | source-safe-docs-qa-metadata | `pnpm run check:v35-gate6` |
| `.proofs/v35/testnet-rollout-readiness-guide.json` | protocol | source-safe-rollout-guide-metadata | `pnpm run check:v35-gate7` |
| `.proofs/v35/telemetry-documentation-interface-integration.json` | protocol | source-safe-interface-integration-metadata | `pnpm run check:v35-gate8` |
| `.proofs/v35/local-staging-telemetry-documentation-rehearsal.json` | protocol | source-safe-rehearsal-metadata | `pnpm run check:v35-gate9` |
| `.proofs/v35/documentation-telemetry-promotion-readiness-report.json` | protocol | source-safe-promotion-readiness-metadata | `pnpm run check:v35-gate10` |

### V35 specifying generated artifacts

Gate 1 requires `.proofs/v35/spec-family-report.json` and `.proofs/v35/canonical-input-report.json` to be declared.
Gate 2 requires `.proofs/v35/documentation-surface-catalog.json` to be generated, source-safe, deterministic, and checked by `pnpm run check:v35-gate2`.
Gate 3 requires `.proofs/v35/telemetry-taxonomy-catalog.json` to be generated, source-safe, deterministic, and checked by `pnpm run check:v35-gate3`.
Gate 4 requires `.proofs/v35/public-docs-usage-guides.json` to be generated, source-safe, deterministic, and checked by `pnpm run check:v35-gate4`.
Gate 5 requires `.proofs/v35/operator-runbook-catalog.json` to be generated, source-safe, deterministic, and checked by `pnpm run check:v35-gate5`.
Gate 6 requires `.proofs/v35/docs-qa-alignment-report.json` to be generated, source-safe, deterministic, and checked by `pnpm run check:v35-gate6`.
Gate 7 requires `.proofs/v35/testnet-rollout-readiness-guide.json` to be generated, source-safe, deterministic, and checked by `pnpm run check:v35-gate7`.
Gate 8 requires `.proofs/v35/telemetry-documentation-interface-integration.json` to be generated, source-safe, deterministic, and checked by `pnpm run check:v35-gate8`.
Gate 9 requires `.proofs/v35/local-staging-telemetry-documentation-rehearsal.json` to be generated, source-safe, deterministic, and checked by `pnpm run check:v35-gate9`.
Gate 10 requires `.proofs/v35/documentation-telemetry-promotion-readiness-report.json` to be generated, source-safe, deterministic, and checked by `pnpm run check:v35-gate10`.
