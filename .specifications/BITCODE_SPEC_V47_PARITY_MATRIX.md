...
- Version: `V47`
- V47 state: canonical promotion complete; V47 parity truth, generated launch artifacts, browser proof, staging-testnet rehearsal, gate closure, and promotion automation are aligned
...
- Scope: V47 canonical parity ledger for commercial website testnet launch readiness over promoted V46 protocol comprehension canon
...
specified, implemented, tested, documented, and proven before V47 promotion closure.
...
| --- | --- | --- | --- | --- |
| Scope and launch freeze | V47 is website-first commercial staging-testnet readiness, not broad feature expansion | closed | Gate 1 opens scope and keeps V46 active canon. | Gate 1 |
...
| Feature excess audit | Remove, hide, flag off, or defer non-launch behavior | closed | `buildV47FeatureExcessAlignmentAudit` and `.proofs/v47/feature-excess-alignment-audit.json` classify launch/supporting/deferred surfaces and check launch CTAs, compatibility redirects, and feature flags. | Gate 2 |
| Seller state machine | IP seller can connect source, synthesize options, review measurements, approve deposit, and track compensation | closed | `buildV47SellerBuyerStateMachineLaw` and `.proofs/v47/seller-buyer-state-machine-law.json` define seller states and guards; `buildV47DepositorWebsiteCompletion` and `.proofs/v47/depositor-website-completion.json` bind the `/deposit` route session, journaled synthesis/review/admission rows, and `/packs` sync. | Gate 3 and Gate 4 |
| Buyer state machine | IP buyer can request Read, approve Need, Finding Fits, preview, settle, receive rights, and get PR delivery | closed | `buildV47SellerBuyerStateMachineLaw` and `.proofs/v47/seller-buyer-state-machine-law.json` define buyer states and guards; `buildV47ReaderWebsiteCompletion` and `.proofs/v47/reader-website-completion.json` bind the `/read` route session, fit measurement review, settlement/rights/delivery ordering, and `/packs` sync. | Gate 3 and Gate 5 |
| `/packs` dashboard | Master-detail PackActivity tracks deposits, reads, proofs, settlements, rights, delivery, compensation, repair | closed | `buildV47PacksAuxillariesCommercialDashboard` and `.proofs/v47/packs-auxillaries-commercial-dashboard.json` bind searchable master-detail rows, settlement/rights/compensation/delivery/repair state readback, proof roots, and the fail-closed repair surface validated by `check:v47-gate6`. | Gate 6 |
...
| E2E IP exchange tests | Browser tests prove selling and buying IP the Bitcode way | closed | `commercial-mvp.ip-exchange.spec.ts` proves the seller deposit flow, buyer measurement/quote/settlement/rights/delivery flow, and `/packs` repair-surface readback in deterministic mock mode, recorded by `buildV47E2eIpSellingBuyingTests` and `.proofs/v47/e2e-ip-selling-buying-tests.json` validated by `check:v47-gate7`. | Gate 7 |
| Landing and public messaging | Landing page explains commercial testnet readiness and user flows | closed | The landing testnet section and docs testnet-meaning card state free BTC-testnet amounts with production-intended behavior, the deposit → read → packs flow, proof-backed trust, and source-safe positioning over preserved V46 claim boundaries, recorded by `buildV47LandingPublicLaunchMessaging` and `.proofs/v47/landing-public-launch-messaging.json` validated by `check:v47-gate8`. | Gate 8 |
| Staging-testnet rehearsal | Canonical deployment validates real routes, real data stores, and BTC-testnet settlement | closed | `buildV47StagingTestnetDeploymentRehearsal` and `.proofs/v47/staging-testnet-deployment-rehearsal.json` bind dry-run lane receipts for the full stack, the realistic-data contract, settlement observation ordering, and blocked mainnet to the deployment truth sources, validated by `check:v47-gate9`; live deployment execution remains operator opt-in. | Gate 9 |
| Promotion readiness | V47 generated artifacts, parity, CI, and promotion workflow are green | closed | `buildV47PromotionReadinessReport` and `.proofs/v47/promotion-readiness-report.json` bind all Gate 2-9 artifacts, V47 promotion scripts, `v47-canon-promotion.yml`, workflow posture, the draft-preview `BITCODE_SPEC_V47_PROVEN.md`, and the prepared post-promotion posture, validated by `check:v47-gate10`. | Gate 10 |
...
| Draft files | V47 SPEC, DELTA, NOTES, and PARITY files exist | closed | Gate 1 creates full draft family. | Gate 1 |
| CI posture | Gate and canon workflows validate active V46 plus draft V47 | closed | Gate 1 wires `check:v47-gate1`. | Gate 1 |
...
| Seller visualization | Depositors see source-safe measurement and compensation basis | closed | `/deposit` renders measurements, criticality, demand, ROI, BTD potential, BTC source-to-shares preview, option roots, compensation estimates, and authority readback validated by `check:v47-gate4`. | Gate 4 |
| Buyer visualization | Readers see source-safe fit measurements and quote basis before paying | closed | `/read` renders Need coverage, Fit confidence, specificity, novelty, reuse, risk, evidence, delivery readiness, selected Fit provenance, final BTD scalar, quote basis, and settlement/rights/delivery readback validated by `check:v47-gate5`. | Gate 5 |
...
| Launch route discipline | Public navigation, landing CTAs, pricing acquisition, and BTD detail paths use `/deposit`, `/read`, or `/packs` rather than `/terminal` or `/exchange` | closed | Gate 2 rewrites launch-facing entrypoints and keeps `/exchange` redirect-only. | Gate 2 |
...
- V47 is active canon; V46 is the prior promoted anchor.
...
V47 Gate 8 is complete when the V47 landing/public launch messaging artifact
...
gate/canon workflows run `check:v47-gate8`.

Gate 10 implementation readback: the promotion readiness report
(`.proofs/v47/promotion-readiness-report.json`) scans every required source,
...
V47 Gate 9 is complete when the V47 staging-testnet rehearsal artifact is
...
workflows run `check:v47-gate9`.

V47 Gate 10 is complete when the V47 promotion readiness artifact is
generated, its package object and tests pass, the V47 promotion scripts,
spec-family and runtime promotion preparation, proven generation, and
`v47-canon-promotion.yml` are wired, gate/canon workflows validate both
pointer postures, the draft-preview `BITCODE_SPEC_V47_PROVEN.md` exists, the
V47 promotion dry-run passes, and gate/canon workflows run
`check:v47-gate10`.
