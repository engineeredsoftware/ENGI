...
- `make shares` now includes a mandatory post-measurement pre-fit Read review: `.bitcode/read-review.json` must expose a reviewable Read with accept, reject, and remeasure-with-feedback outcomes, `/api/read-review` must present and record those decisions for admitted app/API consumers, and fit search is admitted only after the Read is accepted for source-to-shares settlement review
...
- sixth-gate is now closed by `.bitcode/sixth-gate-mvp-closure-proof.json`
