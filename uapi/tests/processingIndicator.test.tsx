...
import { buildProcessingStallLabel } from '@/components/bitcode/execution/pipeline-execution-log';
...
describe('ProcessingIndicator — stall label + amber flip at 180s', () => {
  ...
  it('renders the natural-language sentence label verbatim (pipeline-name prefix trimmed from the agent)', () => {
    ...
    expect(
      screen.getByText(
        'During Discovery, agent Depository Search is Trying, by Reasoning over Prompts · 30s since last update',
      ),
    ).toBeInTheDocument();
  });
