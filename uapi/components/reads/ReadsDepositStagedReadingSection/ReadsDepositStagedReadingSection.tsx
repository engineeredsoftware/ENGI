      ...
      {readNeedRows.length ? (
        <dl className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          {readNeedRows.map((row) => (
            <div key={row.label} className="rounded-none border border-white/8 bg-black/20 px-4 py-3 text-sm">
            ...
            <div className="mt-3 grid gap-2">
              {readNeedStorageProjection.map((record, index) => (
                <div
                  ...
                  className="rounded-none border border-white/8 bg-black/25 px-3 py-2"
                >
