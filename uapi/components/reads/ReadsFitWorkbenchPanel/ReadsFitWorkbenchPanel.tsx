...
export default function ReadsFitWorkbenchPanel({
  ...
  return (
    <article
      ...
      className="mt-5 rounded-none border border-amber-400/18 bg-amber-400/5 px-5 py-5"
    >
      <div className="flex items-start justify-between gap-3">
        ...
        <button
          ...
          className="rounded-none border border-amber-300/30 bg-amber-300/10 px-3 py-2 text-[0.66rem] uppercase tracking-[0.18em] text-amber-100 transition hover:border-amber-200/50 hover:bg-amber-300/15"
        >
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-4">
        {metrics.map((metric) => (
          <div key={`fit-${metric.label}`} className="rounded-none border border-white/8 bg-black/20 px-4 py-4">
      ...
      <dl className="mt-4 grid gap-3 lg:grid-cols-2">
        {rows.map((row) => (
          <div
            key={`fit-${row.label}`}
            className="rounded-none border border-white/8 bg-black/20 px-4 py-4 text-sm"
          >
