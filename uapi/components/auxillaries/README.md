...
Imports **Bitcode** only (never other experiences). Page shell: `uapi/app/auxillaries/`.

Product language: **Pipeline** for runs; **journal** for BTD ledger rows.
**No Terminal product route** (`/terminal` is deleted).
...
`uapi/tests/` (`auxillaries*`, `walletPane*`, `profileStep*`, `externalsPane*`).
