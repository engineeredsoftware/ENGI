/**
 * @bitcode/host-generics — Host primitive contracts (V48 Gate 3).
 *
 * Hierarchy:
 *   Host / BitcodePipelineHost          # this package (primitive)
 *     → LocalHost                       # generic-hosts/Local
 *     → SandboxHost (base)              # abstract sandbox HostKind
 *       → VercelSandboxHost             # generic-hosts/VercelSandbox
 *       → AwsSandboxHost (future)
 *
 * Spec alignment (BITCODE_SPEC_V48 § G3-4 Hosts and provisioning):
 * - HostKind `local` (default): in-process git clone + Node filesystem
 * - HostKind `sandbox`: isolated box; provider vercel | aws
 * - Full checkout on host; only source-safe derivations leave it
