...
import type { AssetPackPreviewBoundary } from '@bitcode/asset-packs-pipelines-syntheses-domain/asset-pack-preview-boundary';
import type { AssetPackSettlementRightsDeliveryBoundary } from '@bitcode/asset-packs-pipelines-domain/asset-pack-settlement-rights-delivery';
...
function laneReadbackFor(laneId: ReadingLocalStagingRehearsalLane): ReadingLocalStagingLaneReadback {
  ...
  const validationCommand = staging
    ...
    : 'pnpm --filter @bitcode/asset-packs-pipelines-syntheses-domain exec jest --config jest.config.cjs --runTestsByPath src/__tests__/reading-local-staging-rehearsal.test.ts --runInBand --forceExit';
  const withoutRoot = {
