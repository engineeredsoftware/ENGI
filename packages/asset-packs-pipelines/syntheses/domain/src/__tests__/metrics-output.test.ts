// @ts-nocheck
/**
 * Bring-up: setup phase streaming + structured DB inserts (phase/agent rows).
 * Opt-in setup runtime under NODE_ENV=test; boundary agents mocked offline.
 */
import assetPack from '../index';
import { Execution } from '@bitcode/execution-generics';
import { enablePipelineStreaming } from '@bitcode/pipelines-generics';

// Mock setup agents to ensure deterministic, fast dry-run (read Setup roster).
jest.mock('../agents/setup/asset-pack-clone-vcs-repository-agent', () => ({
  __esModule: true,
  default: jest.fn().mockResolvedValue({
    success: true,
    repository: { owner: 'acme', name: 'repo', ref: 'main' },
  }),
}));
jest.mock('../agents/setup/asset-pack-initialize-lsp-agent', () => ({
  __esModule: true,
  default: jest.fn().mockResolvedValue({ success: true, lsp: { available: false } }),
}));
      ...
      try {
        ...
        const selectBuilder = (table: string) => {
          ...
          const builder: any = {
            ...
            then: (resolve: any) =>
              Promise.resolve({ data: selectedRows(table, filters), error: null }).then(resolve),
          };
          return builder;
        };
            ...
            return {
              insert: (row: any) => {
                ...
                return {
                  select: (_?: any) => ({
                    single: () =>
                      Promise.resolve({ data: { id: 'id-' + inserts.length } }),
                  }),
                } as any;
              },
        ...
        // Drain async structured write queue before asserting inserts.
        await (streamer as any).flushStructuredWrites?.();
        await new Promise((r) => setImmediate(r));

        expect(res).toBeDefined();

        // Structured inserts occurred (runs + phases + agents).
        expect(
          inserts.some((i) => i.table === 'deliverable_pipeline_runs'),
        ).toBe(true);
        const phaseRows = inserts.filter(
          (i) => i.table === 'deliverable_pipeline_phase_delegations',
        );
        expect(phaseRows.length).toBeGreaterThanOrEqual(1);
        expect(
          phaseRows.some(
            (row) => row.row.phase_name === 'setup' || row.row.phase === 'setup',
          ),
        ).toBe(true);
        const agentRows = inserts.filter(
          (i) => i.table === 'deliverable_pipeline_agent_steps',
        );
        expect(agentRows.length).toBeGreaterThanOrEqual(1);
        expect(
          agentRows.some((r) => String(r.row.agent_name || '').startsWith('setup:')),
        ).toBe(true);
      } finally {
