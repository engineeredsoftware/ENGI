// @ts-nocheck
import { measureAbsoluteObservabilitySurface, ABSOLUTE_MEASUREMENT_KIND } from '../../index';

describe('CORE: measureAbsoluteObservabilitySurface', () => {
  const basePack = {
    title: 'Test DP',
  ...
  it('returns a reading for the absolute kind', () => {
    const result = measureAbsoluteObservabilitySurface({
      dataPack: basePack,
      sources: [
  ...
  it('accepts host staticSignals when provided', () => {
    const result = measureAbsoluteObservabilitySurface({
      dataPack: basePack,
      staticSignals: { 'observability-surface': 0.55 },
    });
    expect(result.measurementKind).toBe(ABSOLUTE_MEASUREMENT_KIND);
