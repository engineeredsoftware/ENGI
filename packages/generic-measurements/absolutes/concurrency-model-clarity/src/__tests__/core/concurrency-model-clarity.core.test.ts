// @ts-nocheck
import { measureAbsoluteConcurrencyModelClarity, ABSOLUTE_MEASUREMENT_KIND } from '../../index';

describe('CORE: measureAbsoluteConcurrencyModelClarity', () => {
  const basePack = {
    title: 'Test DP',
  ...
  it('returns a reading for the absolute kind', () => {
    const result = measureAbsoluteConcurrencyModelClarity({
      dataPack: basePack,
      sources: [
  ...
  it('accepts host staticSignals when provided', () => {
    const result = measureAbsoluteConcurrencyModelClarity({
      dataPack: basePack,
      staticSignals: { 'concurrency-model-clarity': 0.55 },
    });
    expect(result.measurementKind).toBe(ABSOLUTE_MEASUREMENT_KIND);
