/**
 * Bare absolute measure: `concurrency-model-clarity` of a synthesized **DataPack**.
...
export const ABSOLUTE_MEASUREMENT_KIND = 'concurrency-model-clarity' as const;
export const ABSOLUTE_MEASUREMENT_LABEL = 'Concurrency-model clarity' as const;
...
export function measureAbsoluteConcurrencyModelClarity(
  ...
  const fromHost =
    signals && typeof signals['concurrency-model-clarity'] === 'number'
      ? Number(signals['concurrency-model-clarity'])
      : null;
  const volume =
  ...
  return {
    ...
    rationale:
      volume > 0
        ? `Concurrency-model clarity from material identity / static signals: ${volume}.`
        : `Concurrency-model clarity: insufficient evidence (host material identity preferred).`,
    status: volume > 0 ? 'measured' : 'insufficient_evidence',
    policyRole: 'weighted',
  };
}

export default measureAbsoluteConcurrencyModelClarity;
