// @ts-nocheck
import { measureAbsolutePurposeClarity, ABSOLUTE_MEASUREMENT_KIND } from '../../index';

describe('CORE: measureAbsolutePurposeClarity', () => {
  const basePack = {
    title: 'Test DP',
  ...
  it('returns a reading for the absolute kind', () => {
    const result = measureAbsolutePurposeClarity({
      dataPack: basePack,
      sources: [
  ...
  it('accepts host staticSignals when provided', () => {
    const result = measureAbsolutePurposeClarity({
      dataPack: basePack,
      staticSignals: { 'purpose-clarity': 0.55 },
    });
    expect(result.measurementKind).toBe(ABSOLUTE_MEASUREMENT_KIND);
