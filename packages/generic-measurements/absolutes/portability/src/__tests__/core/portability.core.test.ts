// @ts-nocheck
import { measureAbsolutePortability, ABSOLUTE_MEASUREMENT_KIND } from '../../index';

describe('CORE: measureAbsolutePortability', () => {
  const basePack = {
    title: 'Test DP',
  ...
  it('returns a reading for the absolute kind', () => {
    const result = measureAbsolutePortability({
      dataPack: basePack,
      sources: [
  ...
  it('accepts host staticSignals when provided', () => {
    const result = measureAbsolutePortability({
      dataPack: basePack,
      staticSignals: { 'portability': 0.55 },
    });
    expect(result.measurementKind).toBe(ABSOLUTE_MEASUREMENT_KIND);
