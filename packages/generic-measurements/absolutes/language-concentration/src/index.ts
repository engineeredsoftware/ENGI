/**
 * Bare absolute measure: `language-concentration` of a synthesized **DataPack**.
...
export const ABSOLUTE_MEASUREMENT_KIND = 'language-concentration' as const;
export const ABSOLUTE_MEASUREMENT_LABEL = 'Language concentration' as const;
...
export function measureAbsoluteLanguageConcentration(
  ...
  const fromHost =
    signals && typeof signals['language-concentration'] === 'number'
      ? Number(signals['language-concentration'])
      : null;
  const volume =
  ...
  return {
    ...
    rationale:
      volume > 0
        ? `Language concentration from material identity / static signals: ${volume}.`
        : `Language concentration: insufficient evidence (host material identity preferred).`,
    status: volume > 0 ? 'measured' : 'insufficient_evidence',
    policyRole: 'weighted',
  };
}

export default measureAbsoluteLanguageConcentration;
