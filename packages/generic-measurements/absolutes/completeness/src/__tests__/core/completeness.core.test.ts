// @ts-nocheck
import { measureAbsoluteCompleteness, ABSOLUTE_MEASUREMENT_KIND } from '../../index';

describe('CORE: measureAbsoluteCompleteness', () => {
  const basePack = {
    title: 'Test DP',
  ...
  it('is insufficient without host/quality signal (no confidence invention)', () => {
    const result = measureAbsoluteCompleteness({ dataPack: basePack, sources: [{ path: 'src/a.ts', content: 'export const x = 1' }] });
  ...
  it('is measured when host supplies a finite signal', () => {
    const result = measureAbsoluteCompleteness({
      dataPack: basePack,
      staticSignals: { 'completeness': 0.72 },
    });
