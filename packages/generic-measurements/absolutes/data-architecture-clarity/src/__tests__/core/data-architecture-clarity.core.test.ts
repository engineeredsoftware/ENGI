// @ts-nocheck
import { measureAbsoluteDataArchitectureClarity, ABSOLUTE_MEASUREMENT_KIND } from '../../index';

describe('CORE: measureAbsoluteDataArchitectureClarity', () => {
  const basePack = {
    title: 'Test DP',
  ...
  it('returns a reading for the absolute kind', () => {
    const result = measureAbsoluteDataArchitectureClarity({
      dataPack: basePack,
      sources: [
  ...
  it('accepts host staticSignals when provided', () => {
    const result = measureAbsoluteDataArchitectureClarity({
      dataPack: basePack,
      staticSignals: { 'data-architecture-clarity': 0.55 },
    });
    expect(result.measurementKind).toBe(ABSOLUTE_MEASUREMENT_KIND);
