// @ts-nocheck
import { measureAbsoluteCorrectnessEstimate, ABSOLUTE_MEASUREMENT_KIND } from '../../index';

describe('CORE: measureAbsoluteCorrectnessEstimate', () => {
  const basePack = {
    title: 'Test DP',
  ...
  it('is insufficient without host/quality signal (no confidence invention)', () => {
    const result = measureAbsoluteCorrectnessEstimate({ dataPack: basePack, sources: [{ path: 'src/a.ts', content: 'export const x = 1' }] });
  ...
  it('is measured when host supplies a finite signal', () => {
    const result = measureAbsoluteCorrectnessEstimate({
      dataPack: basePack,
      staticSignals: { 'correctness-estimate': 0.72 },
    });
