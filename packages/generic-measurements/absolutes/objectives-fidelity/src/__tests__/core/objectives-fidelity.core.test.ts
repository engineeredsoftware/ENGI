// @ts-nocheck
import { measureAbsoluteObjectivesFidelity, ABSOLUTE_MEASUREMENT_KIND } from '../../index';

describe('CORE: measureAbsoluteObjectivesFidelity', () => {
  const basePack = {
    title: 'Test DP',
  ...
  it('is insufficient without host/quality signal (no confidence invention)', () => {
    const result = measureAbsoluteObjectivesFidelity({ dataPack: basePack, sources: [{ path: 'src/a.ts', content: 'export const x = 1' }] });
  ...
  it('is measured when host supplies a finite signal', () => {
    const result = measureAbsoluteObjectivesFidelity({
      dataPack: basePack,
      staticSignals: { 'objectives-fidelity': 0.72 },
    });
