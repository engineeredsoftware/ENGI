/**
 * Bare absolute measure: `api-style-clarity` of a synthesized **DataPack**.
...
export const ABSOLUTE_MEASUREMENT_KIND = 'api-style-clarity' as const;
export const ABSOLUTE_MEASUREMENT_LABEL = 'API-style clarity' as const;
...
export function measureAbsoluteApiStyleClarity(
  ...
  const fromHost =
    signals && typeof signals['api-style-clarity'] === 'number'
      ? Number(signals['api-style-clarity'])
      : null;
  const volume =
  ...
  return {
    ...
    rationale:
      volume > 0
        ? `API-style clarity from material identity / static signals: ${volume}.`
        : `API-style clarity: insufficient evidence (host material identity preferred).`,
    status: volume > 0 ? 'measured' : 'insufficient_evidence',
    policyRole: 'weighted',
  };
}

export default measureAbsoluteApiStyleClarity;
