// @ts-nocheck
import { measureAbsoluteCopyleftRiskMass, ABSOLUTE_MEASUREMENT_KIND } from '../../index';

describe('CORE: measureAbsoluteCopyleftRiskMass', () => {
  const basePack = {
    title: 'Test DP',
  ...
  it('returns a reading for the absolute kind', () => {
    const result = measureAbsoluteCopyleftRiskMass({
      dataPack: basePack,
      sources: [
  ...
  it('accepts host staticSignals when provided', () => {
    const result = measureAbsoluteCopyleftRiskMass({
      dataPack: basePack,
      staticSignals: { 'copyleft-risk-mass': 0.55 },
    });
    expect(result.measurementKind).toBe(ABSOLUTE_MEASUREMENT_KIND);
