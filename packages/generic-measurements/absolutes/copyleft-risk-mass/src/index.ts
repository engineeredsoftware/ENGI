/**
 * Bare absolute measure: `copyleft-risk-mass` of a synthesized **DataPack**.
...
export const ABSOLUTE_MEASUREMENT_KIND = 'copyleft-risk-mass' as const;
export const ABSOLUTE_MEASUREMENT_LABEL = 'Copyleft risk mass' as const;
...
export function measureAbsoluteCopyleftRiskMass(
  ...
  const fromHost =
    signals && typeof signals['copyleft-risk-mass'] === 'number'
      ? Number(signals['copyleft-risk-mass'])
      : null;
  const volume =
  ...
  return {
    ...
    rationale:
      volume > 0
        ? `Copyleft risk mass from material identity / static signals: ${volume}.`
        : `Copyleft risk mass: insufficient evidence (host material identity preferred).`,
    status: volume > 0 ? 'measured' : 'insufficient_evidence',
    policyRole: 'penalty',
  };
}

export default measureAbsoluteCopyleftRiskMass;
