// @ts-nocheck
import { measureAbsoluteExternalServiceCoupling, ABSOLUTE_MEASUREMENT_KIND } from '../../index';

describe('CORE: measureAbsoluteExternalServiceCoupling', () => {
  const basePack = {
    title: 'Test DP',
  ...
  it('returns a reading for the absolute kind', () => {
    const result = measureAbsoluteExternalServiceCoupling({
      dataPack: basePack,
      sources: [
  ...
  it('accepts host staticSignals when provided', () => {
    const result = measureAbsoluteExternalServiceCoupling({
      dataPack: basePack,
      staticSignals: { 'external-service-coupling': 0.55 },
    });
    expect(result.measurementKind).toBe(ABSOLUTE_MEASUREMENT_KIND);
