 ...
 * Requires REFERENCE_CORPUS fingerprint; empty without host signal.
 */
...
export function measureAbsoluteOriginality(input: DataPackAbsoluteMeasureInput): AbsoluteMeasureResult {
  const raw = input.staticSignals?.['originality'] ?? input.context?.['originality'];
  const n = Number(raw);
