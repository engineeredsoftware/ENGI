/**
 * Bare absolute measure: `framework-surface` of a synthesized **DataPack**.
...
export const ABSOLUTE_MEASUREMENT_KIND = 'framework-surface' as const;
export const ABSOLUTE_MEASUREMENT_LABEL = 'Framework surface' as const;
export const ABSOLUTE_MEASUREMENT_UNIT = 'frameworks' as const;
...
export function measureAbsoluteFrameworkSurface(
  ...
  const fromHost =
    signals && typeof signals['framework-surface'] === 'number'
      ? Number(signals['framework-surface'])
      : null;
  const volume =
  ...
  return {
    ...
    rationale:
      volume > 0
        ? `Framework surface from material identity / static signals: ${volume}.`
        : `Framework surface: insufficient evidence (host material identity preferred).`,
    status: volume > 0 ? 'measured' : 'insufficient_evidence',
    policyRole: 'weighted',
  };
}

export default measureAbsoluteFrameworkSurface;
