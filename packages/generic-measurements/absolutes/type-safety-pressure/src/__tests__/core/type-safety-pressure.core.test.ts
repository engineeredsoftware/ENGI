// @ts-nocheck
import { measureAbsoluteTypeSafetyPressure, ABSOLUTE_MEASUREMENT_KIND } from '../../index';

describe('CORE: measureAbsoluteTypeSafetyPressure', () => {
  const basePack = {
    title: 'Test DP',
  ...
  it('returns a reading for the absolute kind', () => {
    const result = measureAbsoluteTypeSafetyPressure({
      dataPack: basePack,
      sources: [
  ...
  it('accepts host staticSignals when provided', () => {
    const result = measureAbsoluteTypeSafetyPressure({
      dataPack: basePack,
      staticSignals: { 'type-safety-pressure': 0.55 },
    });
    expect(result.measurementKind).toBe(ABSOLUTE_MEASUREMENT_KIND);
