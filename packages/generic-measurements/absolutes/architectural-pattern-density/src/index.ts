/**
 * Bare absolute measure: `architectural-pattern-density` of a synthesized **DataPack**.
...
export const ABSOLUTE_MEASUREMENT_KIND = 'architectural-pattern-density' as const;
export const ABSOLUTE_MEASUREMENT_LABEL = 'Architectural pattern density' as const;
...
export function measureAbsoluteArchitecturalPatternDensity(
  ...
  const fromHost =
    signals && typeof signals['architectural-pattern-density'] === 'number'
      ? Number(signals['architectural-pattern-density'])
      : null;
  const volume =
  ...
  return {
    ...
    rationale:
      volume > 0
        ? `Architectural pattern density from material identity / static signals: ${volume}.`
        : `Architectural pattern density: insufficient evidence (host material identity preferred).`,
    status: volume > 0 ? 'measured' : 'insufficient_evidence',
    policyRole: 'weighted',
  };
}

export default measureAbsoluteArchitecturalPatternDensity;
