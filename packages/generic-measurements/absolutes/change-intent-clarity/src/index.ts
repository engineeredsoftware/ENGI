/**
 * Bare absolute measure: `change-intent-clarity` of a synthesized **DataPack**.
...
export const ABSOLUTE_MEASUREMENT_KIND = 'change-intent-clarity' as const;
export const ABSOLUTE_MEASUREMENT_LABEL = 'Change-intent clarity' as const;
...
export function measureAbsoluteChangeIntentClarity(
  ...
  const fromHost =
    signals && typeof signals['change-intent-clarity'] === 'number'
      ? Number(signals['change-intent-clarity'])
      : null;
  const volume =
  ...
  return {
    ...
    rationale:
      volume > 0
        ? `Change-intent clarity from material identity / static signals: ${volume}.`
        : `Change-intent clarity: insufficient evidence (host material identity preferred).`,
    status: volume > 0 ? 'measured' : 'insufficient_evidence',
    policyRole: 'weighted',
  };
}

export default measureAbsoluteChangeIntentClarity;
