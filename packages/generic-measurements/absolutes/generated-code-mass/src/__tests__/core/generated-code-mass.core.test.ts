// @ts-nocheck
import { measureAbsoluteGeneratedCodeMass, ABSOLUTE_MEASUREMENT_KIND } from '../../index';

describe('CORE: measureAbsoluteGeneratedCodeMass', () => {
  const basePack = {
    title: 'Test DP',
  ...
  it('returns a reading for the absolute kind', () => {
    const result = measureAbsoluteGeneratedCodeMass({
      dataPack: basePack,
      sources: [
  ...
  it('accepts host staticSignals when provided', () => {
    const result = measureAbsoluteGeneratedCodeMass({
      dataPack: basePack,
      staticSignals: { 'generated-code-mass': 0.55 },
    });
    expect(result.measurementKind).toBe(ABSOLUTE_MEASUREMENT_KIND);
