// @ts-nocheck
import { measureAbsoluteCapabilityClarity, ABSOLUTE_MEASUREMENT_KIND } from '../../index';

describe('CORE: measureAbsoluteCapabilityClarity', () => {
  const basePack = {
    title: 'Test DP',
  ...
  it('is insufficient without host/quality signal (no confidence invention)', () => {
    const result = measureAbsoluteCapabilityClarity({ dataPack: basePack, sources: [{ path: 'src/a.ts', content: 'export const x = 1' }] });
  ...
  it('is measured when host supplies a finite signal', () => {
    const result = measureAbsoluteCapabilityClarity({
      dataPack: basePack,
      staticSignals: { 'capability-clarity': 0.72 },
    });
