...
- Every commit and gate pull-request **subject line** must declare its specification/implementation category as a short parenthetical immediately after the version and gate prefix — exactly one of `(spec-only)`, `(impl-only)`, or `(spec-impl)`:
  - `(spec-only)` = specification-only (only `BITCODE_SPEC_*` family / version notes)
  - `(impl-only)` = implementation-only (code, tests, tooling, scripts)
  - `(spec-impl)` = specification-implementation (both in lockstep)
  Examples: `V48 Gate 3 (impl-only): Rename deposit input to Obfuscations` or `V48 Gate 3 (spec-only): Record SDIVF in V48 notes`. A change that touches both kinds is one `(spec-impl)` commit or a split into `(spec-only)` + `(impl-only)` — never an unlabeled mix. This keeps Complete Implementation Derivability (`BITCODE_SPECIFYING.md` §2.8) auditable commit by commit.
