...
const ARTIFACT_PATH = '.proofs/v41/readfitsfinding-prompt-hardening.json';
...
function main() {
  ...
  if (!args.skipPackageTests && failures.length === 0) {
    try {
      run(root, 'node', ['--test', '--test-force-exit', 'scripts/specifying/test/v41-readfitsfinding-prompt-hardening.test.js']);
    } catch (error) {
  ...
  assertCheck(
    ...
    'Roadmap must name V41 Gate 6 or later as current working gate.',
  );
  assertCheck(
    failures,
    ...
    'Roadmap must name V41 Gate 7, Gate 8, or Gate 9 as next.',
  );
