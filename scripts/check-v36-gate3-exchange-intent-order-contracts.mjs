...
const ARTIFACT_PATH = '.proofs/v36/exchange-intent-order-contracts.json';
...
function main() {
  ...
  const requiredFiles = [
    ARTIFACT_PATH,
    'scripts/specifying/src/canonical/exchange-intent-order-contracts.js',
    'scripts/specifying/src/index.js',
    'scripts/specifying/src/index.d.ts',
    'scripts/specifying/test/v36-exchange-intent-order-contracts.test.js',
  ...
  if (failures.length === 0) {
    try {
      run(root, 'node', ['--test', '--test-force-exit', 'scripts/specifying/test/v36-exchange-intent-order-contracts.test.js']);
    } catch (error) {
  ...
  const source = read(root, 'scripts/specifying/src/canonical/exchange-intent-order-contracts.js');
  const index = read(root, 'scripts/specifying/src/index.js');
  const typeDefs = read(root, 'scripts/specifying/src/index.d.ts');
  const test = read(root, 'scripts/specifying/test/v36-exchange-intent-order-contracts.test.js');
