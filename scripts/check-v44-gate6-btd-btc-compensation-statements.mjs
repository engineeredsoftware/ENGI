...
function main() {
  ...
  if (!args.skipPackageTests) {
    try {
      run(root, 'pnpm', ['--dir', 'packages/specifying', 'exec', 'node', '--test', '--test-force-exit', 'test/v44-btd-btc-compensation-statements.test.js']);
    } catch {
      failures.push('packages/specifying/test/v44-btd-btc-compensation-statements.test.js must pass.');
    }
