...
function main() {
  ...
  const requiredFiles = [
    ...
    'packages/specifying/test/v35-telemetry-taxonomy-catalog.test.js',
  ...
  if (failures.length === 0) {
    try {
      run(root, 'pnpm', ['--dir', 'packages/specifying', 'exec', 'node', '--test', '--test-force-exit', 'test/v35-telemetry-taxonomy-catalog.test.js']);
    } catch (error) {
  ...
  const source = read(root, 'packages/specifying/src/canonical/telemetry-taxonomy-catalog.js');
  const index = read(root, 'packages/specifying/src/index.js');
  const typeDefs = read(root, 'packages/specifying/src/index.d.ts');
  const test = read(root, 'packages/specifying/test/v35-telemetry-taxonomy-catalog.test.js');
