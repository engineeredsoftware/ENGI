...
export function specificationPath(repoRoot, relativeName) {
  const cleaned = String(relativeName || '').replace(/^specifications\//u, '');
...
export function bitcodeSpecPointerPath(repoRoot) {
  const under = specificationPath(repoRoot, 'specifications/BITCODE_SPEC.txt');
  if (existsSync(under)) return under;
  const root = path.join(repoRoot, 'specifications/BITCODE_SPEC.txt');
