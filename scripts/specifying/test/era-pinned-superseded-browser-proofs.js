/**
 * Era-pin for proofs superseded by the V48 legacy cockpit browser-proof eradication.
 *
 * V48 Gate 3 eradicated a prior product route cockpit browser proof — the
...
/**
 * True once the legacy cockpit-ux browser proof has been eradicated.
 * Detect via the deleted product path (current product browser proofs live
 * under deposits/reads and must not un-pin historical V39/V40 product proofs).
 */
export const SUPERSEDED = !existsSync(
  path.join(REPO_ROOT, 'apps/uapi/app/ (removed cockpit tree) terminal-ux-browser-proof.ts'),
);

const SKIP_REASON =
  'era-pinned: superseded by V48 Gate 3 legacy cockpit browser-proof eradication (repointed to /deposits, /reads)';
