...
export function buildInferenceProof(outputField, evidenceRefs, promptOrEvaluatorId, modelId = DEFAULT_MODEL_ID) {
  const standIn = modelId === DEFAULT_MODEL_ID;
  return {
    outputField,
    evidenceRefs: summarizeStrings(evidenceRefs),
    promptOrEvaluatorId,
 ...
 *   promptFamilyRegistry?: Record<string, unknown> | null,
...
export function buildPromptCompletenessProof(promptContracts = [], options = {}) {
  ...
  const registryFields = summarizeStrings((/** @type {any} */ (promptFamilyRegistry))?.['registeredPromptOwnedFields'] || []);
  const registeredFields = registryFields.length
    ? registryFields
    : summarizeStrings(promptContracts.flatMap((contract) => contract.outputFields || []));
  ...
  const memberVerdicts = allFields.map((field) => {
    ...
    const familyMember = Array.isArray((/** @type {any} */ (promptFamilyRegistry))?.['promptMembers'])
      ? (/** @type {any} */ (promptFamilyRegistry))['promptMembers'].find((entry) => entry?.memberId === field) || null
      : null;
    ...
    return {
      ...
      inDeclaredFamilyRegistry: !!familyMember || registryFields.length === 0,
      ...
      passed: excludedFields.includes(field)
        ...
        : registeredFields.includes(field)
          && (!!familyMember || registryFields.length === 0)
  ...
  const witnessArtifactPaths = summarizeStrings([
    promptFamilyRegistry ? '.proofs/_shared/prompt-family-registry.json' : null,
  ...
  const replaySteps = [
    buildReplayStep({
      ...
      requiredArtifactPaths: summarizeStrings([promptFamilyRegistry ? '.proofs/_shared/prompt-family-registry.json' : null, '.proofs/_shared/prompt-contracts.json', '.proofs/_shared/prompt-surfaces.json']),
    ...
    buildReplayStep({
      ...
      requiredArtifactPaths: ['.proofs/_shared/prompt-contracts.json', '.proofs/_shared/parsed-completion-envelopes.json'],
    ...
    buildReplayStep({
      ...
      requiredArtifactPaths: ['.proofs/_shared/prompt-surfaces.json'],
    ...
    buildReplayStep({
      ...
      requiredArtifactPaths: ['.proofs/_shared/prompt-surfaces.json', '.proofs/_shared/prompt-contracts.json'],
  ...
  const artifactBindings = summarizeStrings([
    ...
    .concat([
    buildArtifactBinding({ artifactPath: '.proofs/_shared/prompt-contracts.json', role: 'contract', theoremIds: ['prompt_completeness.contract_closure'], requiredForWitness: true, requiredForReplay: true }),
    buildArtifactBinding({ artifactPath: '.proofs/_shared/prompt-surfaces.json', role: 'primary-proof', theoremIds: ['prompt_completeness.coverage_totality', 'prompt_completeness.downstream_consumer_closure', 'prompt_completeness.provenance_truth'], requiredForWitness: true, requiredForReplay: true }),
    buildArtifactBinding({ artifactPath: '.proofs/_shared/parsed-completion-envelopes.json', role: 'supporting-proof', theoremIds: ['prompt_completeness.parsed_envelope_admissibility'], requiredForWitness: true, requiredForReplay: true }),
    buildArtifactBinding({ artifactPath: '.proofs/_shared/prompt-completeness-proof.json', role: 'primary-proof', theoremIds: ['prompt_completeness.witness_replay_closure'], requiredForWitness: true, requiredForReplay: true })
  ]);
  const theoremIds = [
  ...
  const proofClosure = computeProofClosure({
    ...
    replaySteps,
    theoremIds,
    excludeTheoremIds: ['prompt_completeness.witness_replay_closure']
  });
  const theoremVerdicts = [
    ...
    buildTheoremVerdict({
      ...
      failureReasons: proofClosure.allClosed ? [] : ['prompt-completeness witness/replay closure is incomplete']
    })
  ];
  return {
    ...
    proofHash: stableHashObject({
      promptContracts: promptContracts.map((contract) => ({
        promptId: contract.promptId,
        contractHash: contract.contractHash,
        completeness: contract.completeness
      })),
      promptFamilyRegistryHash: (/** @type {any} */ (promptFamilyRegistry))?.['registryHash'] || null
    }),
