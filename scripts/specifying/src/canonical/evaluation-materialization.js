...
export function createEvaluationMaterializationRuntime({
  ...
  function materializeSelectedSourceMaterial(selectedCandidates, branchMode) {
    ...
    for (const candidate of selectedCandidates) {
      ...
      files[`.proofs/source-material/${candidate.assetId}.md`] = `# ${candidate.asset.title}\n\n- assetId: ${candidate.assetId}\n- useTier: ${candidate.useTier}\n- artifactKind: ${candidate.asset.artifactKind}\n- contentRoot: ${candidate.asset.contentRoot}\n- materializationMode: ${candidate.asset.sourceMaterialBinding.mode}\n- confidentiality: ${candidate.asset.sourceMaterialBinding.confidentiality}\n\n${sections}`;
    }
    return files;
  }
  ...
  function assertRequiredBranchArtifacts(branchArtifacts) {
    const requiredPaths = [
      '.proofs/_shared/read.json',
      '.proofs/_shared/read-measurement.json',
      '.proofs/_shared/read-review.json',
      '.proofs/_shared/depositing-surface.json',
      '.proofs/_shared/reading-surface.json',
      '.proofs/_shared/deposit-to-read-surface.json',
      '.proofs/_shared/match-report.json',
      ...
      '.proofs/_shared/policy-release.json',
      '.proofs/_shared/identity-bindings.json',
      '.proofs/_shared/github-boundary.json',
      '.proofs/_shared/artifact-upload-manifest.json',
      '.proofs/_shared/profile-composition.json',
      '.proofs/_shared/prompt-family-registry.json',
      '.proofs/_shared/prompt-surfaces.json',
      '.proofs/_shared/prompt-contracts.json',
      ...
      '.proofs/_shared/verification-receipts.json',
      ...
      '.proofs/_shared/scenario-fixture-manifest.json',
      '.proofs/_shared/test-coverage-report.json',
      '.proofs/_shared/unit-catalog.json',
    ...
    if (branchArtifacts.files['.proofs/_shared/bitcoin-settlement-intent.json']) {
      requiredPaths.push(
    ...
    if (branchArtifacts.files['.proofs/_shared/external-environment-profile.json']) {
      requiredPaths.push(
  ...
  function buildBranchArtifacts({ read, readMeasurement, readReview, benchmarkTarget, branchMode, branchName, depositingSurface, readingSurface, depositingToReadingSurface, matchReport, verificationReport, evalManifest, assetPack, assetPackLock, selectedSourceMaterialManifest, settlementPreview, settlementProof, systemProofBundle, authorizationDecisions, sensitiveDataFlowRecords, policyRelease, assetPackEvidenceManifest, unitCatalog, pipelineTelemetry, selectedCandidates, journalDiff, identityBindings, githubBoundarySurface, artifactUploadManifest, profileCompositionSurface, promptFamilyRegistry, promptSurfaces, promptContracts, inferenceProofs, inferenceMomentContracts, promptImplementationSurface, inferenceSynthesisProof, promptCompletenessProof, parsedCompletionEnvelopes, parsedCompletionEnvelopeArtifact, externalBoundaryManifest, measurementReceipts, staticMeasurementReport, staticMeasurementProof, codeAnalysisFactRegistry, staticHeuristicsRegistry, verificationReceiptsArtifact, verificationDecisionsProof, proofWitnessManifest, selectionConsistencyProof, selectionAndMaterializationProof, identityAuthorizationProof, sensitiveDataFlowProof, authorizationAndSensitiveFlowProof, materializationProof, materializationExclusions, materializationVisibilityProof, sourceToSharesArtifact, settlementParticipationArtifact, accountingPrecisionReport, journalCompletenessProof, settlementSourceToSharesProof, scenarioFixtureManifest, testCoverageReport, projectionPolicy, boundedPublicProof, redactionProof, disclosureProof, disclosureBoundaryProof, proofContract, computeRealityManifest, storageRealityManifest, bitcoinCommitmentManifest, bitcoinTreasuryPolicy, bitcoinAnchor, bitcoinBoundedPublicAnchor, bitcoinSettlementIntent, bitcoinSettlementObservation, bitcoinAuditAnchorProof, bitcoinSettlementInterfaceProof, externalEnvironmentProfile, externalExecutionPolicy, externalTelemetryPolicy, externalTelemetrySummary, networkCapabilityManifest, githubAppBinding, bitcoinNetworkIntent, bitcoinNetworkExecution, bitcoinNetworkObservation, repeatedReadPaymentIntent, repeatedReadPaymentExecution, repeatedReadPaymentObservation, sidechainExecutionReceipt, computeContainerManifest, computeContainerExecution, storageContainerManifest, storagePublicationReceipt, storageRetrievalReceipt, githubLiveSession, githubInventoryFetchReceipt, githubArtifactFetchReceipt, githubBranchPublicationReceipt, githubPrUpdateReceipt, externalRealizationProof, containerRealityProof, githubLiveInterfaceProof }) {
    const files = {
      '.proofs/_shared/read.json': JSON.stringify(read, null, 2),
      '.proofs/_shared/read-measurement.json': JSON.stringify(readMeasurement, null, 2),
      '.proofs/_shared/read-review.json': JSON.stringify(readReview, null, 2),
      '.proofs/_shared/benchmark-target.json': JSON.stringify(benchmarkTarget, null, 2),
      '.proofs/_shared/depositing-surface.json': JSON.stringify(depositingSurface, null, 2),
      '.proofs/_shared/reading-surface.json': JSON.stringify(readingSurface, null, 2),
      '.proofs/_shared/deposit-to-read-surface.json': JSON.stringify(depositingToReadingSurface, null, 2),
      '.proofs/_shared/match-report.json': JSON.stringify(matchReport, null, 2),
      '.proofs/_shared/verification-report.json': JSON.stringify(verificationReport, null, 2),
      '.proofs/_shared/eval-manifest.json': JSON.stringify(evalManifest, null, 2),
      '.proofs/_shared/asset-pack.lock.json': JSON.stringify(assetPackLock, null, 2),
      '.proofs/_shared/selected-source-material.json': JSON.stringify(selectedSourceMaterialManifest, null, 2),
      '.proofs/_shared/settlement-preview.json': JSON.stringify(settlementPreview, null, 2),
      '.proofs/_shared/settlement-proof.json': JSON.stringify(settlementProof, null, 2),
      '.proofs/_shared/journal-diff.json': JSON.stringify(journalDiff, null, 2),
      '.proofs/_shared/system-proof-bundle.json': JSON.stringify(systemProofBundle, null, 2),
      '.proofs/_shared/authorization-decisions.json': JSON.stringify(authorizationDecisions, null, 2),
      '.proofs/_shared/sensitive-data-flow.json': JSON.stringify(sensitiveDataFlowRecords, null, 2),
      '.proofs/_shared/policy-release.json': JSON.stringify(policyRelease, null, 2),
      '.proofs/_shared/identity-bindings.json': JSON.stringify(identityBindings, null, 2),
      '.proofs/_shared/github-boundary.json': JSON.stringify(githubBoundarySurface, null, 2),
      '.proofs/_shared/artifact-upload-manifest.json': JSON.stringify(artifactUploadManifest, null, 2),
      '.proofs/_shared/profile-composition.json': JSON.stringify(profileCompositionSurface, null, 2),
      '.proofs/_shared/prompt-family-registry.json': JSON.stringify(promptFamilyRegistry, null, 2),
      '.proofs/_shared/prompt-surfaces.json': JSON.stringify(promptSurfaces, null, 2),
      '.proofs/_shared/prompt-contracts.json': JSON.stringify(promptContracts, null, 2),
      '.proofs/_shared/inference-moment-contracts.json': JSON.stringify(inferenceMomentContracts, null, 2),
      '.proofs/_shared/inference-proofs.json': JSON.stringify(inferenceProofs || [], null, 2),
      '.proofs/_shared/inference-synthesis-proof.json': JSON.stringify(inferenceSynthesisProof, null, 2),
      '.proofs/_shared/prompt-implementation-surface.json': JSON.stringify(promptImplementationSurface, null, 2),
      '.proofs/_shared/prompt-completeness-proof.json': JSON.stringify(promptCompletenessProof, null, 2),
      '.proofs/_shared/parsed-completion-envelopes.json': JSON.stringify(parsedCompletionEnvelopeArtifact || { envelopes: parsedCompletionEnvelopes || [] }, null, 2),
      '.proofs/_shared/code-analysis-fact-registry.json': JSON.stringify(codeAnalysisFactRegistry, null, 2),
      '.proofs/_shared/static-heuristics-registry.json': JSON.stringify(staticHeuristicsRegistry, null, 2),
      '.proofs/_shared/external-boundary-manifest.json': JSON.stringify(externalBoundaryManifest, null, 2),
      '.proofs/_shared/measurement-receipts.json': JSON.stringify(measurementReceipts, null, 2),
      '.proofs/_shared/static-measurement-report.json': JSON.stringify(staticMeasurementReport, null, 2),
      '.proofs/_shared/static-measurement-proof.json': JSON.stringify(staticMeasurementProof, null, 2),
      '.proofs/_shared/verification-receipts.json': JSON.stringify(verificationReceiptsArtifact, null, 2),
      '.proofs/_shared/verification-decisions-proof.json': JSON.stringify(verificationDecisionsProof, null, 2),
      '.proofs/_shared/selection-consistency-proof.json': JSON.stringify(selectionConsistencyProof, null, 2),
      '.proofs/_shared/selection-and-materialization-proof.json': JSON.stringify(selectionAndMaterializationProof, null, 2),
      '.proofs/_shared/identity-authorization-proof.json': JSON.stringify(identityAuthorizationProof, null, 2),
      '.proofs/_shared/sensitive-data-flow-proof.json': JSON.stringify(sensitiveDataFlowProof, null, 2),
      '.proofs/_shared/authorization-and-sensitive-flow-proof.json': JSON.stringify(authorizationAndSensitiveFlowProof, null, 2),
      '.proofs/_shared/materialization-proof.json': JSON.stringify(materializationProof, null, 2),
      '.proofs/_shared/materialization-exclusions.json': JSON.stringify(materializationExclusions, null, 2),
      '.proofs/_shared/proof-witness-manifest.json': JSON.stringify(proofWitnessManifest, null, 2),
      '.proofs/_shared/materialization-visibility-proof.json': JSON.stringify(materializationVisibilityProof, null, 2),
      '.proofs/_shared/source-to-shares.json': JSON.stringify(sourceToSharesArtifact, null, 2),
      '.proofs/_shared/settlement-participation.json': JSON.stringify(settlementParticipationArtifact, null, 2),
      '.proofs/_shared/accounting-precision-report.json': JSON.stringify(accountingPrecisionReport, null, 2),
      '.proofs/_shared/journal-completeness-proof.json': JSON.stringify(journalCompletenessProof, null, 2),
      '.proofs/_shared/settlement-source-to-shares-proof.json': JSON.stringify(settlementSourceToSharesProof, null, 2),
      '.proofs/_shared/scenario-fixture-manifest.json': JSON.stringify(scenarioFixtureManifest, null, 2),
      '.proofs/_shared/test-coverage-report.json': JSON.stringify(testCoverageReport, null, 2),
      '.proofs/_shared/unit-catalog.json': JSON.stringify(unitCatalog, null, 2),
      '.proofs/_shared/pipeline-telemetry.json': JSON.stringify(pipelineTelemetry, null, 2),
      '.proofs/_shared/projection-policy.json': JSON.stringify(projectionPolicy, null, 2),
      '.proofs/_shared/bounded-public-proof.json': JSON.stringify(boundedPublicProof, null, 2),
      '.proofs/_shared/redaction-proof.json': JSON.stringify(redactionProof, null, 2),
      '.proofs/_shared/disclosure-proof.json': JSON.stringify(disclosureProof, null, 2),
      '.proofs/_shared/disclosure-boundary-proof.json': JSON.stringify(disclosureBoundaryProof, null, 2),
      '.proofs/_shared/proof-contract.json': JSON.stringify(proofContract, null, 2),
      '.proofs/_shared/asset-pack-evidence.json': JSON.stringify(assetPackEvidenceManifest, null, 2),
      ...(computeRealityManifest ? { '.proofs/_shared/compute-reality-manifest.json': JSON.stringify(computeRealityManifest, null, 2) } : {}),
      ...(storageRealityManifest ? { '.proofs/_shared/storage-reality-manifest.json': JSON.stringify(storageRealityManifest, null, 2) } : {}),
      ...(bitcoinCommitmentManifest ? { '.proofs/_shared/bitcoin-commitment-manifest.json': JSON.stringify(bitcoinCommitmentManifest, null, 2) } : {}),
      ...(bitcoinTreasuryPolicy ? { '.proofs/_shared/bitcoin-treasury-policy.json': JSON.stringify(bitcoinTreasuryPolicy, null, 2) } : {}),
      ...(bitcoinAnchor ? { '.proofs/_shared/bitcoin-anchor.json': JSON.stringify(bitcoinAnchor, null, 2) } : {}),
      ...(bitcoinBoundedPublicAnchor ? { '.proofs/_shared/bitcoin-bounded-public-anchor.json': JSON.stringify(bitcoinBoundedPublicAnchor, null, 2) } : {}),
      ...(bitcoinSettlementIntent ? { '.proofs/_shared/bitcoin-settlement-intent.json': JSON.stringify(bitcoinSettlementIntent, null, 2) } : {}),
      ...(bitcoinSettlementObservation ? { '.proofs/_shared/bitcoin-settlement-observation.json': JSON.stringify(bitcoinSettlementObservation, null, 2) } : {}),
      ...(bitcoinAuditAnchorProof ? { '.proofs/_shared/bitcoin-audit-anchor-proof.json': JSON.stringify(bitcoinAuditAnchorProof, null, 2) } : {}),
      ...(bitcoinSettlementInterfaceProof ? { '.proofs/_shared/bitcoin-settlement-interface-proof.json': JSON.stringify(bitcoinSettlementInterfaceProof, null, 2) } : {}),
      ...(externalEnvironmentProfile ? { '.proofs/_shared/external-environment-profile.json': JSON.stringify(externalEnvironmentProfile, null, 2) } : {}),
      ...(externalExecutionPolicy ? { '.proofs/_shared/external-execution-policy.json': JSON.stringify(externalExecutionPolicy, null, 2) } : {}),
      ...(externalTelemetryPolicy ? { '.proofs/_shared/external-telemetry-policy.json': JSON.stringify(externalTelemetryPolicy, null, 2) } : {}),
      ...(externalTelemetrySummary ? { '.proofs/_shared/external-telemetry-summary.json': JSON.stringify(externalTelemetrySummary, null, 2) } : {}),
      ...(networkCapabilityManifest ? { '.proofs/_shared/network-capability-manifest.json': JSON.stringify(networkCapabilityManifest, null, 2) } : {}),
      ...(githubAppBinding ? { '.proofs/_shared/github-app-binding.json': JSON.stringify(githubAppBinding, null, 2) } : {}),
      ...(bitcoinNetworkIntent ? { '.proofs/_shared/bitcoin-network-intent.json': JSON.stringify(bitcoinNetworkIntent, null, 2) } : {}),
      ...(bitcoinNetworkExecution ? { '.proofs/_shared/bitcoin-network-execution.json': JSON.stringify(bitcoinNetworkExecution, null, 2) } : {}),
      ...(bitcoinNetworkObservation ? { '.proofs/_shared/bitcoin-network-observation.json': JSON.stringify(bitcoinNetworkObservation, null, 2) } : {}),
      ...(repeatedReadPaymentIntent ? { '.proofs/_shared/repeated-read-payment-intent.json': JSON.stringify(repeatedReadPaymentIntent, null, 2) } : {}),
      ...(repeatedReadPaymentExecution ? { '.proofs/_shared/repeated-read-payment-execution.json': JSON.stringify(repeatedReadPaymentExecution, null, 2) } : {}),
      ...(repeatedReadPaymentObservation ? { '.proofs/_shared/repeated-read-payment-observation.json': JSON.stringify(repeatedReadPaymentObservation, null, 2) } : {}),
      ...(sidechainExecutionReceipt ? { '.proofs/_shared/sidechain-execution-receipt.json': JSON.stringify(sidechainExecutionReceipt, null, 2) } : {}),
      ...(computeContainerManifest ? { '.proofs/_shared/compute-container-manifest.json': JSON.stringify(computeContainerManifest, null, 2) } : {}),
      ...(computeContainerExecution ? { '.proofs/_shared/compute-container-execution.json': JSON.stringify(computeContainerExecution, null, 2) } : {}),
      ...(storageContainerManifest ? { '.proofs/_shared/storage-container-manifest.json': JSON.stringify(storageContainerManifest, null, 2) } : {}),
      ...(storagePublicationReceipt ? { '.proofs/_shared/storage-publication-receipt.json': JSON.stringify(storagePublicationReceipt, null, 2) } : {}),
      ...(storageRetrievalReceipt ? { '.proofs/_shared/storage-retrieval-receipt.json': JSON.stringify(storageRetrievalReceipt, null, 2) } : {}),
      ...(githubLiveSession ? { '.proofs/_shared/github-live-session.json': JSON.stringify(githubLiveSession, null, 2) } : {}),
      ...(githubInventoryFetchReceipt ? { '.proofs/_shared/github-inventory-fetch-receipt.json': JSON.stringify(githubInventoryFetchReceipt, null, 2) } : {}),
      ...(githubArtifactFetchReceipt ? { '.proofs/_shared/github-artifact-fetch-receipt.json': JSON.stringify(githubArtifactFetchReceipt, null, 2) } : {}),
      ...(githubBranchPublicationReceipt ? { '.proofs/_shared/github-branch-publication-receipt.json': JSON.stringify(githubBranchPublicationReceipt, null, 2) } : {}),
      ...(githubPrUpdateReceipt ? { '.proofs/_shared/github-pr-update-receipt.json': JSON.stringify(githubPrUpdateReceipt, null, 2) } : {}),
      ...(externalRealizationProof ? { '.proofs/_shared/external-realization-proof.json': JSON.stringify(externalRealizationProof, null, 2) } : {}),
      ...(containerRealityProof ? { '.proofs/_shared/container-reality-proof.json': JSON.stringify(containerRealityProof, null, 2) } : {}),
      ...(githubLiveInterfaceProof ? { '.proofs/_shared/github-live-interface-proof.json': JSON.stringify(githubLiveInterfaceProof, null, 2) } : {}),
