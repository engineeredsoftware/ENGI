...
export const V38_PTRR_FAILSAFE_THRICIFIED_STACK_ROWS = Object.freeze([
  row({
    rowId: 'agent:factoryPTRRAgent',
    familyId: 'ptrr_agent_factory',
    label: 'factoryPTRRAgent prompt and step carrier',
  ...
  row({
    ...
    sourceRoots: [SOURCE_ROOTS.generationFactories],
    ptrrStepIds: [...V38_PTRR_STEP_IDS],
    failsafeStageIds: [...V38_FAILSAFE_STAGE_IDS],
    thricifiedGenerationStageIds: [...V38_THRICIFIED_GENERATION_STAGE_IDS],
    ...
    providerCallSlotsPerStep: 9,
    requiredPredicateIds: [
      'generation.creates-failsafe-execution',
      'generation.creates-generation-execution',
      'generation.stores-ptrr-failsafe',
      'generation.stores-ptrr-generation',
      'generation.applies-prompt-overlays',
      'generation.builds-hierarchical-prompt',
      'generation.executes-default-llm',
      'generation.stores-llm-input-prompt-output',
      'generation.stores-parsed-output',
      'generation.maps-sequence-prompts',
    ],
...
function buildPredicateResults(repoRoot, index, gate2Inventory) {
  ...
  predicateResults.push(
    ...
    predicateResult(
      ...
      sourceText.generationFactories.includes('factoryAgentFailsafeGenerationExecution'),
    ),
    predicateResult(
      'generation.creates-generation-execution',
      SOURCE_ROOTS.generationFactories,
      sourceText.generationFactories.includes('factoryAgentThinkingsGenerationExecution'),
    ),
    predicateResult(
      'generation.stores-ptrr-failsafe',
      SOURCE_ROOTS.generationFactories,
    ...
    predicateResult(
      'generation.stores-ptrr-generation',
      SOURCE_ROOTS.generationFactories,
      sourceText.generationFactories.includes("store('ptrr', 'generation'"),
    ),
    predicateResult(
      'generation.applies-prompt-overlays',
      SOURCE_ROOTS.generationFactories,
      sourceText.generationFactories.includes('applyPromptOverlays'),
    ),
    predicateResult(
      'generation.builds-hierarchical-prompt',
      SOURCE_ROOTS.generationFactories,
    ...
    predicateResult(
      'generation.executes-default-llm',
      SOURCE_ROOTS.generationFactories,
      sourceText.generationFactories.includes('getDefaultLLM') && sourceText.generationFactories.includes('await llm(llmInput)'),
    ),
    predicateResult(
      'generation.stores-llm-input-prompt-output',
      SOURCE_ROOTS.generationFactories,
    ...
    predicateResult(
      'generation.maps-sequence-prompts',
      SOURCE_ROOTS.generationFactories,
      sourceText.generationFactories.includes('getSequencePrompt(sequence)')
        && sourceText.generationFactories.includes('PROMPTPART_GENERIC_AGENT_GENERATION_REASON'),
    ),
    predicateResult(
      'gate2.counts-52-ptrr-steps',
