...
function printHelp() {
  process.stdout.write(
    [
      'Usage: node scripts/check-v48-gate4-depositor-website-completion.mjs [--skip-branch-check] [--skip-package-tests] [--skip-uapi-tests] [--repo-root <path>]',
      '',
      'Checks V48 Gate 4 depositor website completion, generated artifact freshness, route journaling, route/protocol bindings, workflows, and focused tests.',
    ].join('\n'),
  );
  process.stdout.write('\n');
}
...
function main() {
  ...
  if (!args.skipBranchCheck) {
    ...
    assertCheck(
      failures,
      branch === 'version/v48' || /^v48\/gate-\d+-[a-z0-9][a-z0-9-]*$/u.test(branch),
      `V48 work must occur on version/v48 or v48/gate-N-* branches. Observed ${branch || 'detached HEAD'}.`,
    );
  }

  for (const relativePath of [
    V48_DEPOSITOR_WEBSITE_COMPLETION_ARTIFACT_PATH,
    'scripts/specifying/src/canonical/v48-depositor-website-completion.js',
    'scripts/specifying/test/v48-depositor-website-completion.test.js',
    'scripts/generate-v48-depositor-website-completion.mjs',
    'scripts/check-v48-gate4-depositor-website-completion.mjs',
    '.specifications/BITCODE_SPEC_V48.md',
    '.specifications/BITCODE_SPEC_V48_DELTA.md',
    '.specifications/BITCODE_SPEC_V48_NOTES.md',
    ...
    'apps/uapi/tests/depositAdmissionActivity.test.ts',
    ...
    assertCheck(failures, exists(root, relativePath), `Missing required V48 Gate 4 file: ${relativePath}`);
  }

  const artifact = buildV48DepositorWebsiteCompletion({ repoRoot: root });
  assertCheck(
    failures,
    artifact.passed,
    `V48 depositor website completion predicates failed: ${artifact.coverage.failedPredicateIds.join(', ')}`,
  );
  ...
  assertCheck(
    failures,
    artifact.coverage.commercialMeasurementReviewComplete === true,
    'Commercial + measurement review must be complete.',
  );
  ...
  assertCheck(
    failures,
    artifact.coverage.exchangeHistoryReadbackComplete === true,
    '/exchange history readback must be complete.',
  );
  assertCheck(
    failures,
    artifact.coverage.tradableDataPackLawComplete === true,
    'Tradable DataPack synthesis law must be complete.',
  );
  ...
  assertCheck(
    failures,
    artifact.coverage.unpaidDataPackSourceVisible === false,
    'Artifact must not expose unpaid DataPack source.',
  );
  ...
  assertCheck(
    failures,
    exists(root, V48_DEPOSITOR_WEBSITE_COMPLETION_ARTIFACT_PATH) &&
      read(root, V48_DEPOSITOR_WEBSITE_COMPLETION_ARTIFACT_PATH) === serialized,
    `${V48_DEPOSITOR_WEBSITE_COMPLETION_ARTIFACT_PATH} must be generated and current.`,
  );
  ...
  assertCheck(
    failures,
    packageJson.includes('"generate:v48-depositor-website-completion"'),
    'package.json must expose generate:v48-depositor-website-completion.',
  );
  assertCheck(
    failures,
    packageJson.includes('"check:v48-depositor-website-completion"'),
    'package.json must expose check:v48-depositor-website-completion.',
  );
  assertCheck(failures, packageJson.includes('"check:v48-gate4"'), 'package.json must expose check:v48-gate4.');
  assertCheck(
    failures,
    gateWorkflow.includes('check-v48-gate4-depositor-website-completion.mjs'),
    'Gate workflow must run V48 Gate 4 checker.',
  );
  assertCheck(
    failures,
    canonWorkflow.includes('check-v48-gate4-depositor-website-completion.mjs'),
    'Canon workflow must run V48 Gate 4 checker.',
  );
  ...
  if (!args.skipPackageTests) {
    try {
      run(root, 'pnpm', [
        ...
        '--test',
        '--test-force-exit',
        'test/v48-depositor-website-completion.test.js',
      ]);
    } catch {
      failures.push('scripts/specifying test/v48-depositor-website-completion.test.js must pass.');
    }
  }
  ...
  if (!args.skipUapiTests) {
    try {
      run(root, 'pnpm', [
        ...
        'tests/depositAdmissionActivity.test.ts',
        '--runInBand',
      ]);
    } catch {
      failures.push(
        'uapi depositRouteModel.test.ts, depositPageClient.test.tsx, and depositAdmissionActivity.test.ts must pass.',
      );
    }
  }

  if (failures.length > 0) {
    process.stderr.write('V48 Gate 4 depositor website completion check failed:\n');
    for (const failure of failures.filter(Boolean)) process.stderr.write(`- ${failure}\n`);
    process.exitCode = 1;
    return;
  }

  process.stdout.write('V48 Gate 4 depositor website completion check passed.\n');
}
