...
- `apps/uapi/app/terminal/TerminalOperatorCard.tsx`
- `apps/uapi/app/terminal/terminal-operator-explainers.ts`
- `apps/uapi/app/terminal/TerminalExperienceFrame.tsx`
- `apps/uapi/app/terminal/TerminalCommandDeck.tsx`
- `apps/uapi/app/terminal/TerminalActionWorkbenchCard.tsx`
- `apps/uapi/app/terminal/TerminalExternalInterfacingPanel.tsx`
- `apps/uapi/app/terminal/TerminalDepositReadWorkbench.tsx`
- `apps/uapi/app/terminal/TerminalRepositoryContextPanel.tsx`
- `apps/uapi/app/terminal/TerminalCoreNativeSections.tsx`
- `apps/uapi/app/terminal/TerminalClosureNativeSections.tsx`
- `apps/uapi/app/terminal/TerminalLiveSummaryStrip.tsx`
- `apps/uapi/app/terminal/TerminalTransactionWorkspace.tsx`
- `apps/uapi/app/terminal/TerminalTransactionDetailActionBar.tsx`
- `apps/uapi/app/terminal/TerminalWorkspaceRail.tsx`
- `apps/uapi/app/terminal/terminal-closure-state.ts`
- `apps/uapi/app/terminal/terminal-external-runtime.ts`
- `apps/uapi/app/terminal/terminal-live-summary.ts`
- `apps/uapi/app/terminal/terminal-deposit-read-workbench.ts`
- `apps/uapi/app/terminal/terminal-repository-context.ts`
...
- `apps/uapi/app/api/vcs/[provider]/*`
...
- proof surface:
  ...
  retained MCP proof carriers are now explicit too: `packages/mcp-generics/src/mcp-server/src/__tests__/{tools/MCPToolsTestSuite.test.ts,integration/mcp-server.test.ts,unit/auth.test.ts}` must stay green against the current server surface and must not emit lingering Jest open-handle warnings, with imported singleton intervals handled as real runtime teardown obligations rather than ignored test noise
  ...
  active AssetPack package primitives are explicit fifth-gate witnesses too: `packages/asset-packs-pipelines/domain/src/types/AssetPackWrittenAssetType.ts`, `packages/asset-packs-pipelines/domain/src/semantic-resolution.ts`, `packages/asset-packs-pipelines/domain/src/tools/search.ts`, and `apps/uapi/tests/searchRelevantAssetPackEvidence.test.ts` must keep written-asset type ownership and prior-run evidence search named as Bitcode AssetPack behavior, with `deliverableType` and vector-RPC naming treated only as compatibility payload/storage detail
  ...
  package-by-package prompt consumer mapping is now also explicit in `protocol-demonstration/V26_PROMPT_SURFACES.md`, and active app configs such as `apps/uapi/tsconfig.json` no longer keep `@bitcode/prompts/src/*` compatibility aliases on the live path
  ...
  fifth-gate active-product naming witnesses now also cover live public-shell style carriers and retained package surfaces such as `@bitcode/external-apps-chatgpt`, `@bitcode/web-search`, `@bitcode/api/streams`, and `@bitcode/email`, with the expectation that user-facing strings, active identifiers, package docs, and canonical package comments all converge on Bitcode naming rather than lingering old-brand residue
  fifth-gate active-product naming witnesses now also cover mounted public-shell teaching carriers and retained package docs such as `apps/uapi/app/(root)/components/{MarketingFeaturesGrid,MarketingComputeSection,MarketingCompetitorTableSection,MarketingMarketplaceSection}.tsx`, `apps/uapi/app/orbitals/components/OrbitalsConnectsOrbitalEmailConnection.tsx`, and `packages/{web-search,registry}/*`, so public demo-like terminal strings, active classnames/comments, and retained registry/web-search docs cannot silently drift back to old-brand wording
  ...
  fifth-gate active-product naming witnesses now also cover retained shared contracts such as `@bitcode/errors` and direct consumers like `packages/api/src/routes/shippables.ts`, compatibility `packages/api/src/routes/deliverables.ts`, and `apps/uapi/app/error.tsx`, so exported symbols, docs, and active error teaching cannot silently keep old branded error-contract naming after promotion
  fifth-gate active-product naming witnesses now also cover live webhook and mock-system carriers such as `apps/uapi/app/api/webhook/{route,verify}.ts`, `apps/uapi/tests/webhookRoute.test.ts`, `apps/uapi/mocking/index.ts`, `apps/uapi/mocking/integration/MockProvider.tsx`, `apps/uapi/mocking/generators/ComprehensiveMockDataGenerators.ts`, `apps/uapi/mocking/{README.md,INTEGRATION_GUIDE.md,QUICK_START_*.md,COMPREHENSIVE_SYSTEM_SUMMARY.md,validate-system.js}`, and `apps/uapi/mocking/scripts/*`, so trigger labels, comment commands, browser debug globals, shipped operator docs, and script ownership paths cannot silently drift back to `engi-*`, `__engiMock*`, or stale `app/mocking` product truth
  fifth-gate active-product naming witnesses now also cover canonical auxillary-pane ownership and retained LLM harness config, including `apps/uapi/app/auxillaries/components/AuxillariesSurface.tsx`, `apps/uapi/app/auxillaries/components/Auxillaries{Profile,Connects,Interfaces,BTD}Pane.tsx`, `apps/uapi/app/orbitals/components/Orbitals{Profile,Connects,Interfaces,BTD}Pane.tsx`, `packages/llm-generics/{jest.config.cjs,tsconfig.test.json,__mocks__/*,README.md}`, `jest.base.cjs`, and `pnpm-workspace.yaml`, so active route ownership and workspace test/runtime config cannot silently preserve orbitals-pane implementation ownership, `engi-*` mock filenames, `@engi` transform allowlists, or dead Engi-era workspace entries
  fifth-gate active-product naming witnesses now also cover shipped JS/runtime carry-through and Storybook/docs carriers such as `packages/{errors,logger,observability}/*.js`, `packages/{security,multimodal-utils,tech-types,supabase}/*`, and active `apps/uapi/.storybook/stories/*`, so generated runtime artifacts, package docs/comments, and operator-facing story/example copy cannot silently preserve `@engi/*`, `Engi`, or Engi-branded sample values after Bitcode promotion
  fifth-gate active-product naming witnesses now also cover canonical auxillary step aliases and retained teaching/example carriers such as `apps/uapi/app/auxillaries/components/Auxillaries{Credits,Connects,Models,ProfileStep}.tsx`, active test imports under `apps/uapi/tests/{creditsStep*,connectionsStep*,modelsStep*,profileStep*,orbitals*}.tsx`, retained docs/examples in `packages/{doc-comment,figma,generic-tools,system-grep,testing,executions-mcp}/*`, and active e2e/story fixtures like `apps/uapi/tests/e2e/conversation-digest-pipeline.test.ts`, `apps/uapi/tests/e2e/fixtures/stories/killBug.json`, and `apps/uapi/.storybook/stories/{SourceDivider,SourceConfig}.stories.tsx`, so fifth-gate proof covers canonical auxillary ownership in tests and the remaining sample-product teaching layer rather than stopping at runtime/package code alone
  fifth-gate active-product naming witnesses now also cover the Bitcode root spec/toolchain family and the auxiliary-focused story/test import layer, so `BITCODE_SPEC*.md`, `BITCODE_SPEC.txt`, `scripts/*bitcode*.mjs`, and active Storybook/test carriers such as `apps/uapi/.storybook/stories/{LoginPane,OnboardingPane,user/Auth,user/Onboarding}.stories.*` plus `apps/uapi/tests/{orbitalsPaneTabs,orbitalsWorkspacePanels,orbitalPaneMeta}.test.*` cannot silently regress to root `ENGI_SPEC*` paths or compatibility `orbitals` imports when canonical auxillary carriers already exist
  ...
  fifth-gate active-product naming witnesses now also cover canonical auxillary lower-level implementation carriers such as `apps/uapi/app/auxillaries/components/{headers/*,shared/*,models/*,AuxillariesDataSharingPanel.tsx,auxillary-pane-explainers.ts,profile-pane.module.css}`, so canonical pane files do not silently keep orbitals-owned header/shared/model/data-share/explainer imports after the auxillary surface has already become the active owner
  fifth-gate active-product naming witnesses now also cover retained orbitals lower-level compatibility carriers such as `apps/uapi/app/orbitals/components/{headers/*,shared/*,models/*,OrbitalsDataSharingPanel.tsx,orbital-pane-explainers.ts}`, so those paths remain thin compatibility re-exports rather than silently preserving duplicate live implementations after canonical auxillary equivalents exist
  fifth-gate active-product naming witnesses now also cover canonical auxillary onboarding/data contracts and active external-realization witnesses such as `apps/uapi/app/auxillaries/auxillary-onboarding-contract.ts`, `apps/uapi/app/api/auxillaries/{onboarding,data}/route.ts`, `apps/uapi/hooks/useUserData.ts`, `apps/uapi/app/auxillaries/components/AuxillariesSurface.tsx`, `apps/uapi/tests/{orbitalsOnboardingRoute,userDataRoute,api/externalRealizationRoute,terminalExternalRuntime}.test.ts`, and active Bitcode operating docs like `protocol-demonstration/{CHECKLIST,SCRIPT,BITCODE_DEMO_SPEC_V15,SPEC_V6_GAP_ANALYSIS}.md`, so payload aliases, schema-backed onboarding semantics, env witnesses, and branch-artifact naming cannot silently preserve `models/credits`, `ENGI_V24_*`, or `ENGI_READ.md`
  ...
  fifth-gate proof now also covers the canonical `$BTD` package/terminal balance surface through `packages/{btd,api,digest,web-search,orm}/*`, `apps/uapi/{app/api/auxillaries,app/terminal,app/executions,tests,components/base/bitcode/{btd,execution,inputs},stories/BTDInvestmentExperience.stories.tsx,.env.example,README.md,app/orbitals/README.md}/*`, and `protocol-demonstration/test/v26-active-product-naming.test.js`, so active Bitcode package imports, ORM model owners, route payloads, measured-BTD transaction/workspace processing stats, BTD share/read-right carriers, and public-shell wording do not silently regress to credit-bucket semantics.
  fifth-gate proof now also covers canonical in-product BTD acquisition and BTD-notification execution through `apps/uapi/components/base/bitcode/btd/{btd-tracker,BTDPrices}.tsx`, `apps/uapi/app/(root)/components/MarketingPricingSection.tsx`, `/auxillaries/btd`, `apps/uapi/networking/api-client.ts`, and `supabase/templates/{low_btd_reminder,out_of_btd,btd_transfer}.html`, so the live BTC→BTD acquisition/balance/transfer flow no longer routes through `app/_legacy`, checkout/Stripe route owners, or old `credit-transfer` / `low-credits-reminder` / `out-of-credits` naming.
  fifth-gate proof now also covers canonical organization treasury and interface-readiness posture through `apps/uapi/app/{auxillaries/components/organization/{BTDTreasuryManagement,OrganizationSettings}.tsx,orbitals/components/organization/OrganizationSettings.tsx,(root)/components/MarketingSetupForm.tsx}` and `protocol-demonstration/test/v26-active-product-naming.test.js`, so active organization-facing treasury guidance stays wallet/BTC/`$BTD`/Connects-native, Stripe-backed credit purchase scaffolding stays deleted, and `apps/uapi/app/_legacy` remains absent.
  fifth-gate proof now also covers canonical auxillary runtime selectors through `apps/uapi/app/auxillaries/components/{AuxillariesProvider,AuxillariesSurface,AuxillariesContent}.tsx`, `apps/uapi/app/auxillaries/components/shared/AuxillariesPaneTabs.tsx`, `apps/uapi/components/base/bitcode/nav/AuxillariesUseButton.tsx`, `apps/uapi/styles/{orbital-global,orbital}.css`, `apps/uapi/tests/auxillariesProvider.test.tsx`, and the active auxillary Playwright specs, so the live runtime/test interface uses `auxillaries-*` portal/open/button/label selectors and portal ids rather than preserving `data-orbital-testid`, `orbital-portal`, `orbital-open`, `orbital-label-active`, or `__orbitalPrefetched`.
  fifth-gate proof now also covers active E2E onboarding and auxillary proof carriers through `apps/uapi/tests/e2e/{auxillaries.helpers,auxillaries.profile,auxillaries.connections.flows,auxillaries.interfaces.flows,auxillaries.navigation,auxillaries.btd.flows,auxillaries.btd.interval.active,auxillaries.btd.chart.nodata,account.btd.visual,onboarding.visual,onboarding-full-flow}.spec.ts`, so active Playwright filenames, onboarding-state helpers, test titles, and snapshot ids teach auxillaries and `$BTD` rather than preserving `orbital.*` and `credits.*` as active Bitcode product truth.
  fifth-gate proof now also covers devex-side package and mock closure through `apps/uapi/package.json`, absent `apps/uapi/{package-lock.json,pnpm-lock.yaml}`, and `apps/uapi/mocking/*`, so nested lockfiles, deleted checkout remnants, and `USER_CREDITS` / `ORGANIZATION_CREDITS` / `CREDIT_TRANSACTIONS` cannot silently reintroduce Stripe or credit-era truth after the live Bitcode surface has been cut to wallet/BTC/`$BTD`.
  fifth-gate proof now also covers MCP-side `$BTD` holding-read and BTC-fee settlement semantics through `packages/mcp-generics/src/mcp-server/src/{types,auth/middleware,pipeline-execution/adapter,tools/pipeline-tools,tools/orchestration-tools,resources/*,__tests__/*,docs/openapi-generator.ts,server.ts}`, requiring `btdBalance`, `manageBtdHoldings`, `minimumBtdHolding`, `measuredBtdEstimate`, `measuredBtd`, `feeAsset`, and BTC-fee reference fields across auth, execution, metrics, fixtures, and published machine-interface surfaces rather than preserving `creditBalance`, `manageCredits`, `estimatedBtd`, `btdUsed`, or `creditsUsed` as live Bitcode truth.
  fifth-gate proof now also covers Bitcode acquisition and asset-pack execution storytelling through `apps/uapi/app/(root)/components/MarketingBtdInvestmentExperience.tsx`, `packages/mcp-generics/src/mcp-server/{ARCHITECTURE.md,docs/public/mcp-overview.md,src/__tests__/fixtures/MCPTestFixtures.ts,src/__tests__/tools/MCPToolsTestSuite.test.ts}`, `apps/uapi/.storybook/stories/conversations/{RichTextInput,Pickers,PipelineIntegration,CompleteSystem}.stories.tsx`, and `apps/uapi/tests/api/deliverables.test.ts`, so wallet-gated BTC settlement, GitHub-before-transacting posture, asset-pack execution, and `$BTD` planning remain the active interface story rather than Stripe checkout or credit-era investment narration.
  ...
  fifth-gate active-product naming witnesses now also cover the canonical auxillary-onboarding contract and reserved read-measurement teaching layer such as `apps/uapi/app/{terminal/terminal-run-data.ts,api/auxillaries/onboarding/route.ts,api/auxillaries/notifications/_shared.ts}`, `apps/uapi/hooks/useConversationStream.ts`, `apps/uapi/mocking/middleware/MockMiddleware.ts`, `apps/uapi/tests/{orbitalsOnboardingRoute,terminalTransactions}.test.ts`, and `internal-docs/{BITCODE_AUXILLARIES_READINESS,BITCODE_CONNECTED_SERVICES,BITCODE_FRONTEND_ARCHITECTURE,BITCODE_EXCHANGE_DATABASE,BITCODE_API,BITCODE_EXECUTIONS,BITCODE_AGENTIC_EXECUTION,BITCODE_INTERFACE_STYLE}.md`, so active Bitcode proof also guards the canonical pane payloads, unauthorized onboarding response shape, and read-measurement placeholder posture instead of leaving stale step-only or Measure wording behind in code/docs/tests
...
- terminal/API proof closure for `protocol-demonstration/public/app.js`, `protocol-demonstration/server.js`, `apps/uapi/app/api/read-review/route.ts`, `apps/uapi/app/api/make-bitcode-branch/route.ts`, `apps/uapi/app/terminal/TerminalReadScenarioPanel.tsx`, `apps/uapi/app/terminal/terminal-closure-state.ts`, `TerminalClosureNativeSections.tsx`, `terminal-section-atlas.ts`, `terminal-transaction-detail.ts`, and `terminal-activity-history.ts` so review admission and quantized fit-quality rows remain reviewable, writable, parity-tested against protocol behavior, and readable through the Bitcode Terminal and app API rather than only through raw artifacts
...
Required coverage posture:
- spec:
  MVP elevation obligations are explicit and closed by `.bitcode/sixth-gate-mvp-closure-proof.json`
- tests:
  MVP product paths are exercised as one coherent Bitcode product rather than a minimum-functional bring-up baseline
- proof surface:
  sixth-gate MVP closure proof explicitly names what was elevated beyond fifth-gate minimum functionality
...
- `apps/uapi/app/terminal/terminal-shell-bridge.tsx`
  test coverage:
  `apps/uapi/tests/terminalShellBridge.test.tsx`
  proof surface:
...
- `apps/uapi/app/api/executions/_shared.ts`
  test coverage:
  `apps/uapi/tests/api/executionsHistoryRoute.test.ts`, `apps/uapi/tests/api/executionsHistoryRunRoute.test.ts`, and `apps/uapi/tests/usePipelineExecution.test.tsx`
  proof surface:
...
- `apps/uapi/app/api/executions/history/[runId]/route.ts`
  test coverage:
  `apps/uapi/tests/api/executionsHistoryRunRoute.test.ts` and `apps/uapi/tests/usePipelineExecution.test.tsx`
  proof surface:
...
- `apps/uapi/app/auxillaries/components/AuxillariesProvider.tsx`
  test coverage:
  `apps/uapi/tests/auxillariesProvider.test.tsx` plus live `/terminal` verification with auxillaries entry exercised from the running app
  proof surface:
...
- `apps/uapi/app/api/auxillaries/user/data-share/route.ts`
  test coverage:
...
- `apps/uapi/components/base/bitcode/layout/nav.tsx`
  test coverage:
  `apps/uapi/tests/navWorkspaceChrome.test.tsx` plus live `/terminal` verification
  proof surface:
...
- `apps/uapi/app/orbitals/components/OrbitalsLoginPane.tsx`, `apps/uapi/app/orbitals/components/OrbitalsContent.tsx`, `apps/uapi/app/orbitals/components/shared/OrbitalsPaneTabs.tsx`, `apps/uapi/app/orbitals/components/shared/OrbitalsWorkspacePanels.tsx`, `apps/uapi/app/orbitals/OrbitalsRouteClient.tsx`
  test coverage:
  `apps/uapi/tests/orbitalsLoginPane.test.tsx`, `apps/uapi/tests/orbitalsContent.test.tsx`, `apps/uapi/tests/orbitalsWorkspacePanels.test.tsx`, `apps/uapi/tests/orbitalsRouteClient.test.tsx`, and `apps/uapi/tests/orbitalPaneMeta.test.ts`
  proof surface:
...
- `apps/uapi/components/base/bitcode/layout/workspace-surface.ts`
  test coverage:
...
- `apps/uapi/app/terminal/terminal-command-state.ts`
  test coverage:
  `apps/uapi/tests/terminalCommandState.test.ts`
  proof surface:
...
- `apps/uapi/app/terminal/TerminalCommandDeck.tsx`, `apps/uapi/app/terminal/TerminalLiveSummaryStrip.tsx`, `apps/uapi/app/terminal/TerminalSectionAtlas.tsx`, `apps/uapi/app/terminal/TerminalPreservedShellSurface.tsx`, `apps/uapi/app/terminal/TerminalTransactionWorkspace.tsx`, `apps/uapi/app/terminal/terminal-run-data.ts`, `apps/uapi/app/orbitals/components/OrbitalsInterfacesPane.tsx`, `apps/uapi/app/orbitals/components/OrbitalsBTDPane.tsx`, `apps/uapi/app/orbitals/components/OrbitalsModelsPane.tsx`, `apps/uapi/components/base/bitcode/notifications/notification-presentation.ts`, `apps/uapi/components/base/bitcode/layout/user-menu.tsx`
  test coverage:
  `apps/uapi/tests/terminalCommandPresentation.test.ts`,
  `apps/uapi/tests/terminalFlowGuide.test.ts`,
  `apps/uapi/tests/terminalWorkspaceRailCard.test.tsx`,
  `apps/uapi/tests/orbitalsInterfacesPane.test.tsx`,
  `apps/uapi/tests/orbitalsBTDPane.test.tsx`,
...
- `apps/uapi/app/terminal/TerminalOperatorCard.tsx`
  test coverage:
  `apps/uapi/tests/terminalOperatorCard.test.tsx`
  proof surface:
...
- `README.md`, `apps/uapi/README.md`, `protocol-demonstration/README.md`, `apps/uapi/app/terminal/README.md`, `apps/uapi/app/auxillaries/README.md`, `apps/uapi/app/orbitals/README.md`, `apps/uapi/components/base/bitcode/README.md`, `apps/uapi/components/base/bitcode/execution/README.md`
  test coverage:
...
- `apps/uapi/app/(root)/components/MarketingLandingPage.tsx`, `apps/uapi/app/(root)/components/landing/MarketingLandingHero.tsx`, `apps/uapi/app/(root)/components/landing/MarketingLandingPillarCard.tsx`, `apps/uapi/app/(root)/components/landing/MarketingLandingGuideCard.tsx`, `apps/uapi/app/(root)/components/landing/MarketingLandingTerminalPreview.tsx`, `apps/uapi/app/(root)/components/landing/marketing-landing-shared.tsx`, `apps/uapi/components/base/bitcode/layout/footer.tsx`, `apps/uapi/components/base/bitcode/layout/bitcode-public-copy.ts`, `apps/uapi/app/(root)/components/PublicDocsPageContent.tsx`, `apps/uapi/app/docs/page.tsx`, `apps/uapi/app/demo-video/page.tsx`, `apps/uapi/app/(root)/components/MarketingOperatorGuideCard.tsx`, `apps/uapi/app/(root)/components/marketing-operator-guide-assets.ts`
  test coverage:
  `apps/uapi/tests/marketingLandingPage.test.tsx`, `apps/uapi/tests/footerPublicShell.test.tsx`, `apps/uapi/tests/marketingOperatorGuideCard.test.tsx`, `apps/uapi/tests/publicDocsPageContent.test.tsx`, and `apps/uapi/tests/e2e/landing.mobile-scroll.spec.ts`
  proof surface:
...
- `apps/uapi/app/(root)/components/PublicShellFrame.tsx`, `apps/uapi/components/base/bitcode/layout/nav.tsx`, `apps/uapi/components/base/bitcode/layout/NavBrand.tsx`, `apps/uapi/components/base/bitcode/layout/workspace-surface.ts`, `apps/uapi/components/base/bitcode/layout/footer.tsx`, `apps/uapi/components/base/bitcode/layout/bitcode-public-explainers.ts`
  test coverage:
  `apps/uapi/tests/navPublicShell.test.tsx`, `apps/uapi/tests/navBrand.test.tsx`, and `apps/uapi/tests/navWorkspaceChrome.test.tsx`
  proof surface:
...
- `apps/uapi/app/terminal/TerminalWorkspaceRailCard.tsx`
  test coverage:
  `apps/uapi/tests/terminalWorkspaceRailCard.test.tsx`
  proof surface:
...
- `apps/uapi/app/terminal/terminal-live-summary.ts`
  test coverage:
  `apps/uapi/tests/terminalLiveSummary.test.ts`
  proof surface:
...
- `apps/uapi/app/terminal/terminal-section-atlas.ts`
  test coverage:
  `apps/uapi/tests/terminalSectionAtlas.test.ts`
  proof surface:
...
- `apps/uapi/app/terminal/TerminalSectionAtlas.tsx`
  test coverage:
...
- `apps/uapi/app/terminal/TerminalSupplySelectionPanel.tsx`
  test coverage:
  `apps/uapi/tests/terminalSupplySelection.test.ts` for normalized supply-selection state plus lint and live `/terminal` verification for the shared operator-shell framing
  proof surface:
...
- `apps/uapi/app/terminal/terminal-core-surface.ts`
  test coverage:
  `apps/uapi/tests/terminalCoreSurface.test.ts`
  proof surface:
...
- `apps/uapi/app/terminal/TerminalActionWorkbenchCard.tsx`
  test coverage:
  `apps/uapi/tests/terminalActionWorkbenchCard.test.tsx`
  proof surface:
...
- `apps/uapi/app/terminal/terminal-transactions.ts`
  test coverage:
  `apps/uapi/tests/terminalTransactions.test.ts`
  proof surface:
...
- `packages/api/src/executions/agentic-execution.ts`, `packages/api/src/routes/executions.ts`, `apps/uapi/app/terminal/TerminalPageClient.tsx`, and `apps/uapi/components/base/bitcode/layout/{bitcode-public-copy,bitcode-public-explainers}.ts`
  test coverage:
  `apps/uapi/tests/{terminalTransactions,navPublicShell,footerPublicShell,marketingLandingPage,marketingOperatorGuideCard,edgetimesPageContent,conversationsRouteClient,orbitalsRouteClient}.test.*`
  proof surface:
...
- `packages/api/src/executions/agentic-execution.ts`, `packages/api/src/routes/executions.ts`, `apps/uapi/networking/api-client.ts`, `apps/uapi/hooks/useExecutionState.ts`, `apps/uapi/app/executions/components/ExecutionsPageClient.tsx`, and `apps/uapi/components/base/bitcode/layout/sidebars/left-sidebar.tsx`
  test coverage:
  focused lint on retained execution carriers plus `apps/uapi/tests/terminalSurfaceCopy.test.ts`
  proof surface:
...
- `apps/uapi/app/terminal/terminal-workspace-explainers.ts`, `apps/uapi/app/terminal/{TerminalCommandDeck.tsx,TerminalTransactionWorkspace.tsx,terminal-flow-guide.ts,terminal-command-presentation.ts,TerminalCoreNativeSections.tsx,TerminalDepositComposer.tsx,TerminalPreservedShellSurface.tsx}`, `apps/uapi/components/base/bitcode/execution/{BitcodeTransactionsTable.tsx,bitcode-transaction-explainers.ts}`, and `apps/uapi/app/auxillaries/components/{auxillary-pane-explainers.ts,AuxillariesBTDPane.tsx}`
  test coverage:
  `apps/uapi/tests/terminalSurfaceCopy.test.ts`
  proof surface:
...
- `apps/uapi/app/terminal/{terminal-run-data.ts,TerminalLiveSummaryStrip.tsx,TerminalTransactionActivitySurface.tsx,terminal-workspace-copy.ts}`, `apps/uapi/app/auxillaries/components/auxillary-pane-meta.ts`, and `apps/uapi/app/api/conversations/_shared.ts`
  test coverage:
  `apps/uapi/tests/{terminalTransactions,terminalTransactionDetail,terminalTransactionDetailSnapshot,terminalTransactionSource,bitcodeDetailPanel}.test.*`
  proof surface:
...
- `BITCODE_SPEC_V26.md`, `apps/uapi/app/terminal/{TerminalDepositReadWorkbench.tsx,TerminalReadScenarioPanel.tsx,TerminalClosureNativeSections.tsx,terminal-workspace-copy.ts,page.tsx}`, and `apps/uapi/app/auxillaries/components/{AuxillariesConnectsPane.tsx,auxillary-pane-meta.ts,headers/AuxillariesConnectsPaneHeader.tsx}`
  test coverage:
  `apps/uapi/tests/{terminalSurfaceCopy,connectionsStep.static}.test.*`
  proof surface:
  ...
  active V26 specification and live deposit/read/connects carriers now teach Bitcode as protocol, product, and proofs across `Bitcode Mainnet`, the `Bitcode Terminal`, read measurement, asset-pack synthesis, settlement follow-through, and wallet-plus-GitHub transacting prerequisites instead of generic transaction or deliverable posture
- `apps/uapi/app/terminal/{TerminalTransactionDetailSurface.tsx,TerminalTransactionDetailHero.tsx,TerminalTransactionIdentityCard.tsx,TerminalTransactionClosureCard.tsx,TerminalTransactionProofsCard.tsx,TerminalTransactionHistoryCard.tsx,TerminalTransactionDetailActionBar.tsx,TerminalClosureControlDeck.tsx,TerminalTransactionActivitySurface.tsx,TerminalExperienceFrame.tsx,TerminalWorkspaceRail.tsx,TerminalMockTransactionDetails.tsx,terminal-command-presentation.ts,terminal-flow-guide.ts,README.md}`, `apps/uapi/components/base/bitcode/execution/{BitcodeTransactionsTable.tsx,BitcodeTransactionsActiveFilters.tsx,BitcodeTransactionsOverview.tsx,README.md}`, and `apps/uapi/app/terminal/terminal-transaction-detail.ts`
  test coverage:
  `apps/uapi/tests/{terminalTransactionDetail,terminalTransactionDetailCards,terminalCommandPresentation,terminalWorkspaceRailCard,bitcodeTransactionsOverview,bitcodeDetailRowList,bitcodePayloadRowsCard,bitcodePayloadDetailCard,bitcodeDetailPanel}.test.*`
  proof surface:
...
- `apps/uapi/app/terminal/TerminalTransactionsTable.tsx`
  test coverage:
...
- `apps/uapi/app/terminal/terminal-transaction-source.ts`
  test coverage:
  `apps/uapi/tests/terminalTransactionSource.test.ts`
  proof surface:
...
- `apps/uapi/components/base/bitcode/execution/bitcode-transaction-active-filters.ts`
  test coverage:
  `apps/uapi/tests/protocol-demonstrationTransactionActiveFilters.test.ts`
  proof surface:
...
- `apps/uapi/components/base/bitcode/execution/BitcodeTransactionsActiveFilters.tsx`
  test coverage:
  `apps/uapi/tests/protocol-demonstrationTransactionsActiveFilters.test.tsx`
  proof surface:
...
- `apps/uapi/app/terminal/terminal-transaction-detail.ts`
  test coverage:
  `apps/uapi/tests/terminalTransactionDetail.test.ts`
  proof surface:
...
- `apps/uapi/app/terminal/TerminalTransactionClosureCard.tsx`
  test coverage:
...
- `apps/uapi/app/terminal/TerminalTransactionProofsCard.tsx`
  test coverage:
  `apps/uapi/tests/terminalTransactionDetailCards.test.tsx`
  proof surface:
  explicit proofs detail carrier inside selected-transaction detail
- `apps/uapi/app/terminal/TerminalTransactionHistoryCard.tsx`
  test coverage:
  `apps/uapi/tests/terminalTransactionDetailCards.test.tsx`
  proof surface:
...
- `apps/uapi/components/base/bitcode/execution/BitcodeDetailCollection.tsx`
  test coverage:
  `apps/uapi/tests/protocol-demonstrationDetailCollection.test.tsx`
  proof surface:
...
- `apps/uapi/components/base/bitcode/execution/BitcodeDetailPanel.tsx`
  test coverage:
  `apps/uapi/tests/protocol-demonstrationDetailPanel.test.tsx`
  proof surface:
...
- `apps/uapi/components/base/bitcode/execution/BitcodeTransactionsFilterBar.tsx`
  test coverage:
  lint plus `apps/uapi/tests/terminalTransactions.test.ts` and `apps/uapi/tests/protocol-demonstrationTransactionsFilterBar.test.tsx`
  proof surface:
...
- `apps/uapi/components/base/bitcode/execution/bitcode-transaction-data-mode.ts`
  test coverage:
  `apps/uapi/tests/protocol-demonstrationTransactionDataMode.test.ts`
  proof surface:
...
- `apps/uapi/components/base/bitcode/execution/BitcodePayloadShape.tsx`
  test coverage:
  `apps/uapi/tests/protocol-demonstrationPayloadShape.test.tsx`
  proof surface:
...
- `apps/uapi/components/base/bitcode/execution/BitcodePayloadTree.tsx`
  test coverage:
  `apps/uapi/tests/protocol-demonstrationPayloadTree.test.tsx`
  proof surface:
...
- `apps/uapi/components/base/bitcode/execution/BitcodeChipCloud.tsx`
  test coverage:
  browser verification and card-level integration through `apps/uapi/tests/terminalTransactionDetailCards.test.tsx`
  proof surface:
...
- `apps/uapi/components/base/bitcode/execution/BitcodeActionPillRow.tsx`
  test coverage:
  `apps/uapi/tests/protocol-demonstrationActionPillRow.test.tsx`
  proof surface:
...
- `apps/uapi/components/base/bitcode/execution/bitcode-transaction-explainers.ts`
  test coverage:
  covered indirectly through `apps/uapi/tests/protocol-demonstrationInlineExplainer.test.tsx`, `apps/uapi/tests/protocol-demonstrationTransactionsFilterBar.test.tsx`, `apps/uapi/tests/protocol-demonstrationPayloadInspector.test.tsx`, `apps/uapi/tests/protocol-demonstrationPayloadShape.test.tsx`, and `apps/uapi/tests/protocol-demonstrationPayloadTree.test.tsx`
  proof surface:
...
- `apps/uapi/components/base/bitcode/execution/BitcodePayloadDetailCard.tsx`
  test coverage:
  `apps/uapi/tests/protocol-demonstrationPayloadDetailCard.test.tsx`
  proof surface:
...
- `apps/uapi/components/base/bitcode/execution/BitcodePayloadRowsCard.tsx`
  test coverage:
  `apps/uapi/tests/protocol-demonstrationPayloadRowsCard.test.tsx`
  proof surface:
...
- `apps/uapi/components/base/bitcode/execution/BitcodePayloadCollectionCard.tsx`
  test coverage:
  `apps/uapi/tests/protocol-demonstrationPayloadCollectionCard.test.tsx`
  proof surface:
...
- `apps/uapi/components/base/bitcode/execution/bitcode-transaction-types.ts`
  test coverage:
  imported through `apps/uapi/tests/terminalTransactions.test.ts`, `apps/uapi/tests/terminalTransactionQuery.test.ts`, and `apps/uapi/tests/protocol-demonstrationTransactionsPagination.test.tsx`
  proof surface:
  typed base-component contract for transaction-master filtering, overview, pagination, and row-selection carriers
- `apps/uapi/app/terminal/terminal-supply-selection.ts`
  test coverage:
  `apps/uapi/tests/terminalSupplySelection.test.ts`
  proof surface:
...
- `apps/uapi/app/terminal/terminal-deposit-composer.ts`
  test coverage:
  `apps/uapi/tests/terminalDepositComposer.test.ts`
  proof surface:
...
- `apps/uapi/app/terminal/terminal-read-scenarios.ts`
  test coverage:
  `apps/uapi/tests/terminalReadScenarios.test.ts`
  proof surface:
...
- `apps/uapi/app/terminal/TerminalReadScenarioPanel.tsx`
  test coverage:
  lint plus localhost `/terminal` verification with route-local scenario selection mounted, `apps/uapi/tests/api/{readReviewRoute.test.ts,readReviewProtocolParity.test.ts}`, and `protocol-demonstration/test/v26-read-review-source-to-shares.test.js`
  proof surface:
...
- `apps/uapi/app/terminal/terminal-closure-controls.ts`
  test coverage:
  `apps/uapi/tests/terminalClosureControls.test.ts`
  proof surface:
...
- `apps/uapi/app/terminal/TerminalClosureControlDeck.tsx`
  test coverage:
...
- `apps/uapi/app/terminal/terminal-repository-context.ts`
  test coverage:
  `apps/uapi/tests/terminalRepositoryContext.test.ts`
  proof surface:
...
- `apps/uapi/app/terminal/terminal-run-activity.ts`
  test coverage:
...
- `apps/uapi/components/base/bitcode/activity/bitcode-activity-model.ts`
  test coverage:
...
- `apps/uapi/app/executions/page.tsx`, `apps/uapi/app/executions/[runId]/page.tsx`, `apps/uapi/app/auxillaries/[pane]/page.tsx`, `apps/uapi/app/orbitals/*/page.tsx`
  test coverage:
...
- `apps/uapi/app/terminal/TerminalTransactionDetailHero.tsx`
  test coverage:
  lint plus `apps/uapi/tests/terminalTransactionDetail.test.ts`
  proof surface:
  reusable overview carrier for selected-transaction detail SRP decomposition
- `apps/uapi/app/terminal/TerminalTransactionIdentityCard.tsx`
  test coverage:
  lint plus `apps/uapi/tests/terminalTransactionDetail.test.ts`
  proof surface:
...
- `packages/asset-packs-pipelines/domain/{src/preprocess.ts,src/__dev__/bringup-demo.ts,src/__tests__/metrics-output.test.ts,src/agents/setup/asset-pack-setup-plan-agent.ts,src/agents/finish/asset-pack-finish-create-pull-request-delivery-agent.ts}`
  test coverage:
  `protocol-demonstration/test/v26-active-product-naming.test.js`
  proof surface:
  fifth-gate AssetPack pipeline closure so active bring-up, default branch naming, and LLM env contracts read as Bitcode instead of their old-brand predecessors
- `packages/parsing/src/parsing.ts`
  test coverage:
  `protocol-demonstration/test/v26-active-product-naming.test.js`
  proof surface:
  fifth-gate parsing/runtime closure so prompt logging obeys the Bitcode env/debug contract
- `packages/externals/vercel/{src/index.ts,README.md}`
  test coverage:
  `protocol-demonstration/test/v26-active-product-naming.test.js`
  proof surface:
...
- `apps/chatgpt/{src/{tools.ts,server.ts,__tests__/{tools.test.ts,yapperFlow.test.ts}},README.md,DEMO.md,TLDR.md}`
  test coverage:
  `apps/chatgpt/src/__tests__/{tools.test.ts,yapperFlow.test.ts}`
  proof surface:
...
- `apps/uapi/{config/features.ts,app/hero-client.tsx,app/(root)/components/MarketingScreenshotSection.tsx,styles/conversations.css,styles/orbital.css,components/base/bitcode/layout/footer.tsx,app/fill-gaps.tsx,tests/footerPublicShell.test.tsx}`
  test coverage:
  `apps/uapi/tests/footerPublicShell.test.tsx`, targeted `next lint`, and `protocol-demonstration/test/v26-active-product-naming.test.js`
  proof surface:
...
- `packages/{conversations-generics/src/agent/ConversationAgent.ts,lsp/src/index.ts,files/src/securityUtils.ts,generic-llms/src/providers/{google.ts,openai.ts,anthropic.ts},generic-llms/tests/unit/registry.test.ts,web-search/src/orchestrator.ts} + apps/uapi/hooks/useConversationStream.ts`
  test coverage:
...
- `packages/{api/src/routes/user.ts,doc-code/jest.config.cjs,doc-code/__mocks__/protocol-demonstration-tools-generics.ts,generic-doc-comment-plugins/doc-developing/package.json,supabase/src/{index.js,streams.js}} + apps/uapi/{app/executions/hooks/usePersistedState.ts,hooks/usePatternRecognition.ts,mocking/generators/ComprehensiveMockDataGenerators.ts}`
  test coverage:
  `apps/uapi/tests/{apiKeysRoutes,mcpSmoke}.test.ts`, targeted `next lint`, and `protocol-demonstration/test/v26-active-product-naming.test.js`
  proof surface:
...
- `{Makefile,docs/api/conversations-openapi.yaml,packages/email/README.md,apps/uapi/.env.example,internal-docs/{BITCODE_AUXILLARIES_READINESS,BITCODE_FRONTEND_ARCHITECTURE,BITCODE_EXCHANGE_DATABASE}.md,.ai/PRODUCT.md}`
  test coverage:
  ...
  proof surface:
  fifth-gate helper/documentation closure so root helpers, OpenAPI metadata, onboarding docs, internal architecture guides, and package usage examples teach Bitcode/auxillaries/current V26 onboarding and app posture instead of preserving old-brand, compatibility-route, or V26 wording in active proof-bearing carriers
...
- `{PRODUCT.md,internal-docs/{BITCODE_TERMINAL_OPERATOR_EXPERIENCE,INTEGRATIONS,BITCODE_CONVERSATIONS,BITCODE_API}.md,scripts/phase2-complete.sh}`
  test coverage:
  `protocol-demonstration/test/v26-active-product-naming.test.js`
  proof surface:
...
- `{internal-docs/{BITCODE_CONNECTED_SERVICES,DOC-CODING,BITCODE_PROMPT_SYSTEM}.md,packages/mcp-generics/src/mcp-server/src/types/index.ts,packages/orm/src/types/database.generated.ts}`
  test coverage:
  `protocol-demonstration/test/v26-active-product-naming.test.js`
  proof surface:
...
- `{packages/mcp-generics/src/mcp-server/{docs/public/{mcp-overview,mcp-api-reference}.md,src/docs/{mcp-spec-generator,openapi-generator}.ts,deployment/kubernetes/deployment.yaml},packages/security/README.md,packages/testing/src/prompt-quality-framework/README.md,packages/web-search/README.md}`
  test coverage:
  `protocol-demonstration/test/v26-active-product-naming.test.js`
  proof surface:
...
- `{internal-docs/{TERMINOLOGY,BITCODE_ARCHITECTURE_PATTERNS,PERFORMANCE,BITCODE_VERIFICATION,BITCODE_PROTOCOL_THESIS}.md,apps/uapi/ARCHITECTURE.md}`
  test coverage:
  `protocol-demonstration/test/v26-active-product-naming.test.js`
  proof surface:
...
- `{apps/uapi/app/conversations/{ConversationsRouteClient.tsx,README.md},apps/uapi/app/conversations/components/{ConversationsOverlay.tsx,ConversationsEnhancedRichTextInput.tsx,ConversationsList.tsx,ConversationsSidebarLogs.tsx},apps/uapi/app/api/conversations/_shared.ts,apps/uapi/tests/{conversationsRouteClient,conversationMockShared}.test.ts*}`
  test coverage:
  `apps/uapi/tests/{conversationsRouteClient,conversationMockShared}.test.ts*` and `protocol-demonstration/test/v26-active-product-naming.test.js`
  proof surface:
...
- `{packages/api/src/routes/conversations.ts,packages/api/src/conversations/{messages.ts,attachments.ts},apps/uapi/tests/{api/conversationThreadStreamRoute.test.ts,api/chatStreamRoute.test.ts,conversationMessages.test.ts}}`
  test coverage:
  `apps/uapi/tests/api/conversationThreadStreamRoute.test.ts`, `apps/uapi/tests/api/chatStreamRoute.test.ts`, and `apps/uapi/tests/conversationMessages.test.ts`
  proof surface:
...
- `{packages/api/src/{routes/conversations.ts,conversations/conversations.ts},apps/uapi/app/api/conversations/[conversationId]/route.ts,apps/uapi/hooks/{useConversationPages.ts,useConversationStream.ts},apps/uapi/app/conversations/components/{ConversationsOverlay.tsx,hooks/useChatState.ts},apps/uapi/tests/{api/conversationsRouteRead.test.ts,useConversationStream.firstWrite.test.ts}}`
  test coverage:
  `apps/uapi/tests/api/conversationsRouteRead.test.ts` and `apps/uapi/tests/useConversationStream.firstWrite.test.ts`
  proof surface:
...
- `{apps/uapi/app/conversations/components/{ConversationsEnhancedRichTextInput.tsx,ConversationsOverlay.tsx,conversation-chat-mapping.ts},apps/uapi/tests/{conversationsEnhancedRichTextInput.test.tsx,conversationsOverlayMapping.test.ts}}`
  test coverage:
  `apps/uapi/tests/conversationsEnhancedRichTextInput.test.tsx` and `apps/uapi/tests/conversationsOverlayMapping.test.ts`
  proof surface:
...
- `{apps/uapi/app/terminal/{TerminalPageClient.tsx,TerminalCommandDeck.tsx,TerminalDepositComposer.tsx,TerminalClosureControlDeck.tsx,TerminalTransactionWorkspace.tsx,TerminalTransactionDetailSurface.tsx,terminal-activity-history.ts},apps/uapi/tests/terminalActivityHistory.test.ts}`
  test coverage:
...
- `{apps/uapi/app/terminal/{TerminalPageClient.tsx,TerminalDepositReadWorkbench.tsx,TerminalReadScenarioPanel.tsx,TerminalSupplySelectionPanel.tsx,TerminalActionWorkbenchCard.tsx,terminal-activity-history.ts},apps/uapi/tests/terminalActivityHistory.test.ts}`
  test coverage:
  `apps/uapi/tests/terminalActivityHistory.test.ts`
  proof surface:
...
- `{apps/uapi/app/terminal/{TerminalPageClient.tsx,TerminalRepositoryContextPanel.tsx,TerminalExternalInterfacingPanel.tsx,terminal-activity-history.ts},apps/uapi/tests/terminalActivityHistory.test.ts}`
  test coverage:
  `apps/uapi/tests/terminalActivityHistory.test.ts`
  proof surface:
...
- `{apps/uapi/lib/bitcode-transaction-readiness.ts,apps/uapi/app/terminal/{TerminalPageClient.tsx,TerminalCommandDeck.tsx,TerminalClosureControlDeck.tsx,TerminalDepositComposer.tsx,TerminalFlowGuideCard.tsx,terminal-flow-guide.ts},apps/uapi/app/auxillaries/components/AuxillariesConnectsPane.tsx,apps/uapi/tests/{bitcodeTransactionReadiness.test.ts,terminalFlowGuide.test.ts}}`
  test coverage:
  `apps/uapi/tests/bitcodeTransactionReadiness.test.ts` and `apps/uapi/tests/terminalFlowGuide.test.ts`
  proof surface:
...
- `{packages/orm/src/{profile-contract.ts,models/user-profiles.ts},packages/api/src/routes/{auxillaries.ts,user.ts,auth.ts},apps/uapi/app/api/auxillaries/profile/route.ts,apps/uapi/app/auxillaries/components/{AuxillariesSurface.tsx,AuxillariesProfilePane.tsx,AuxillariesBTDPane.tsx},apps/uapi/hooks/useUserData.ts,apps/uapi/tests/{userProfileRoute.test.ts,userDataRoute.test.ts}}`
  test coverage:
  `apps/uapi/tests/userProfileRoute.test.ts` and `apps/uapi/tests/userDataRoute.test.ts`
  proof surface:
...
- `{apps/uapi/components/base/bitcode/execution/BitcodeInlineExplainer.tsx,apps/uapi/app/terminal/{terminal-workspace-explainers.ts,TerminalExperienceFrame.tsx,TerminalCommandDeck.tsx,TerminalRepositoryContextPanel.tsx,TerminalSupplySelectionPanel.tsx,TerminalReadScenarioPanel.tsx,TerminalDepositComposer.tsx,TerminalClosureControlDeck.tsx},apps/uapi/tests/{terminalSurfaceCopy.test.ts,bitcodeInlineExplainer.test.tsx}}`
  test coverage:
  `apps/uapi/tests/terminalSurfaceCopy.test.ts` and `apps/uapi/tests/bitcodeInlineExplainer.test.tsx`
  proof surface:
...
- `{apps/uapi/app/terminal/{TerminalPageClient.tsx,TerminalTransactionWorkspace.tsx,terminal-transaction-source.ts,terminal-protocol-projection.ts,terminal-run-data.ts},apps/uapi/tests/{terminalTransactionSource.test.ts,terminalProtocolProjection.test.ts}}`
  test coverage:
  `apps/uapi/tests/terminalTransactionSource.test.ts` and `apps/uapi/tests/terminalProtocolProjection.test.ts`
  proof surface:
...
- `{protocol-demonstration/{server.js,src/{bitcode-demo.js,bitcode-runtime.js,index.d.ts},test/v26-read-review-source-to-shares.test.js},apps/uapi/{app/api/{read-review,state,make-bitcode-branch}/route.ts,app/terminal/TerminalReadScenarioPanel.tsx,lib/bitcode-app-context.ts,tests/api/{readReviewRoute.test.ts,readReviewProtocolParity.test.ts}}}`
  test coverage:
  `node --test protocol-demonstration/test/v26-read-review-source-to-shares.test.js`, `apps/uapi/tests/api/{readReviewRoute.test.ts,readReviewProtocolParity.test.ts}`, and `pnpm exec tsc --noEmit --pretty false --project tsconfig.json` from `uapi`
  proof surface:
...
- `{apps/uapi/app/terminal/{TerminalClosureNativeSections.tsx,terminal-closure-state.ts,terminal-transaction-detail-snapshot.ts,terminal-activity-history.ts},apps/uapi/tests/{terminalClosureState.test.ts,terminalTransactionDetailSnapshot.test.ts,terminalActivityHistory.test.ts}}`
  test coverage:
  `apps/uapi/tests/{terminalClosureState.test.ts,terminalTransactionDetailSnapshot.test.ts,terminalActivityHistory.test.ts}`
  proof surface:
...
- `{apps/uapi/app/terminal/{TerminalCommandDeck.tsx,TerminalClosureControlDeck.tsx,TerminalTransactionDetailSurface.tsx,terminal-activity-history.ts,terminal-transaction-detail-snapshot.ts,terminal-transaction-detail.ts},apps/uapi/tests/{terminalActivityHistory.test.ts,terminalTransactionDetailSnapshot.test.ts,terminalTransactionDetail.test.ts}}`
  test coverage:
  `apps/uapi/tests/terminalActivityHistory.test.ts`, `apps/uapi/tests/terminalTransactionDetailSnapshot.test.ts`, and `apps/uapi/tests/terminalTransactionDetail.test.ts`
  proof surface:
...
- `{packages/api/src/{executions/agentic-execution.ts,conversations/streaming.ts,routes/{auth.ts,shippables.ts}},packages/vcs-generics/src/service.ts,apps/uapi/{hooks/useConversationStream.ts,types/{next-server-compat.d.ts,supabase-js-compat.d.ts},tsconfig.json},apps/uapi/tests/{useConversationStream.firstWrite.test.ts,api/{chatStreamRoute.test.ts,conversationThreadStreamRoute.test.ts}}}`
  test coverage:
  `apps/uapi/tests/useConversationStream.firstWrite.test.ts`, `apps/uapi/tests/api/chatStreamRoute.test.ts`, and `apps/uapi/tests/api/conversationThreadStreamRoute.test.ts`
  proof surface:
...
- `{packages/api/src/routes/shippables.ts,packages/asset-packs-pipelines/domain/src/agents/{design/iterate-product-md-agent.ts,digest/capture-learnings-agent.ts,validation/asset-pack-ready-to-instruct-agent.ts}}`
  test coverage:
...
- `{packages/api/src/conversations/{attachments.ts,conversations.ts},apps/uapi/components/base/bitcode/{execution/{github-selectors.tsx,pipeline-execution-log.tsx,TagOverflowList.tsx},icons/LogoIcon.tsx,typing-animation.tsx,layout/{nav.tsx,sidebars/left-sidebar.tsx}},apps/uapi/config/features.ts,apps/uapi/tests/{navPublicShell.test.tsx,navWorkspaceChrome.test.tsx}}`
  test coverage:
  `apps/uapi/tests/navPublicShell.test.tsx` and `apps/uapi/tests/navWorkspaceChrome.test.tsx`
  proof surface:
...
- `{packages/api/src/vcs/github-service.ts,packages/artifacts/src/artifacts.ts,packages/browser-storage/src/{persisted-state.ts,storage-manager.ts},apps/uapi/types/{supabase-js-compat.d.ts,react-compat.d.ts}}`
  test coverage:
...
- `{packages/prompts/src/{index.ts,raw_promptparts/**/*},packages/mcp-generics/{src/index.ts,README.md},packages/asset-packs-pipelines/domain/src/agents/setup/initialize-mcps-tools-agent.ts}`
  test coverage:
  filtered `tsc` verification over the active `uapi` program for the prompt-space and MCP primitive corridor
  proof surface:
  fifth-gate prompt-and-execution primitive compile-health closure so the raw promptpart reservoir no longer compiles as a duplicate-import global-script swarm, live execution/pipeline prompt classes no longer route through the retained prompt barrel, and the retained `@bitcode/mcp-generics` primitive no longer drags an unnecessary schema dependency into the active compile path, keeping prompt-system explicitness and MCP config normalization admissible as Bitcode-owned primitives
- `{packages/execution-generics/src/{index.ts,executors/conditional_executor.ts},packages/agent-generics/src/index.ts,packages/pipelines-generics/src/{index.ts,phases/phase-factory.ts,phases/sdivf-factory.ts,gate-system/meta-phase-orchestrator.ts},packages/asset-packs-pipelines/domain/package.json,packages/mcp-generics/src/mcp-server/{src/index.ts,package.json},packages/prompts/src/raw_promptparts/specific/promptpart_specific_pipeline_deliverable_purpose_corestatement.ts}`
  test coverage:
  filtered `tsc` verification over the active `uapi` program for the execution-core package corridor
  proof surface:
  fifth-gate execution primitive/reference admissibility closure so canonical Bitcode execution primitives remain teachable reusable infrastructure, while retained SDIVF/PTRR/meta-phase and AssetPack compatibility families are explicitly contained as reference orchestration instead of being narrated as the live Bitcode product implementation
- `{packages/mcp-generics/{README.md,src/mcp-server/{README.md,src/{index.ts,server.ts,types/index.ts,pipeline-execution/adapter.ts},docs/{public/mcp-overview.md,openapi/bitcode-mcp-openapi.json},package.json}}}`
  test coverage:
  filtered `tsc` verification over the active `uapi` program for the MCP interface corridor
  proof surface:
  fifth-gate MCP interface-precision closure so primitive `@bitcode/mcp-generics` utilities stay distinct from the Bitcode Exchange-facing MCP server, the active docs/source carriers teach `Bitcode Protocol` / `Bitcode Exchange` / `Bitcode Terminal` as one canonical hierarchy, third-party MCPs plus repository/provider connections and attachments are modeled as ingress/input context, and retained compatibility outputs are normalized toward Bitcode asset-pack meaning instead of remaining generic egress
- `{packages/mcp-generics/src/mcp-server/src/{types/index.ts,tools/pipeline-tools.ts,pipeline-execution/adapter.ts,__tests__/unit/pipeline-ingress-contract.test.ts}}`
  test coverage:
...
- `{packages/mcp-generics/src/mcp-server/src/{prompts/{workflow-prompts,development-prompts}.ts,tools/pipeline-tools.ts,__tests__/unit/prompt-asset-pack-canon.test.ts}}`
  test coverage:
  `packages/mcp-generics/src/mcp-server/src/__tests__/unit/prompt-asset-pack-canon.test.ts`
  proof surface:
  fifth-gate Bitcode MCP prompt-canon closure so rendered workflow/development prompts and the compatibility-mounted pipeline tool description teach Bitcode asset-pack pipeline execution over source-to-shares needs, keep `bitcode://pipelines/asset-pack/create` plus subtype names as compatibility surfaces, and prevent old output-pipeline wording from remaining active MCP canon
- `{apps/uapi/app/api/webhook/{route,verify}.ts,apps/uapi/tests/{webhookRoute.test.ts,api/webhookSignature.test.ts}}`
  test coverage:
  `apps/uapi/tests/webhookRoute.test.ts` and `apps/uapi/tests/api/webhookSignature.test.ts`
  proof surface:
  fifth-gate webhook-ingress closure so GitHub labels and `@bitcode-*` comments schedule asset-pack pipeline automation as ingress-only Bitcode activity, emit `Trigger Asset-Pack Pipeline` telemetry with `github_webhook` ingress and `asset_packs` output meaning, and use `bitcode-asset-pack-trigger` as the active command
- `apps/uapi/hooks/useExecutionState.{ts,js}`
  test coverage:
...
- `{apps/uapi/app/terminal/{TerminalTransactionActivitySurface.tsx,TerminalTransactionDetailSurface.tsx,terminal-activity-history.ts,terminal-transaction-detail-snapshot.ts,terminal-transaction-detail.ts},apps/uapi/tests/{terminalTransactionActivitySurface.test.tsx,terminalActivityHistory.test.ts,terminalTransactionDetailSnapshot.test.ts,terminalTransactionDetail.test.ts}}`
  test coverage:
  `apps/uapi/tests/terminalTransactionActivitySurface.test.tsx`, `apps/uapi/tests/terminalActivityHistory.test.ts`, `apps/uapi/tests/terminalTransactionDetailSnapshot.test.ts`, and `apps/uapi/tests/terminalTransactionDetail.test.ts`
  proof surface:
...
- `{packages/vcs-generics/src/{connections.ts,provider.ts}}`
  test coverage:
...
- `{packages/generic-tools/{mcps-tools/jira/src/index.ts,multimodal-processing/src/index.ts,use-computer/src/index.ts},packages/asset-packs-pipelines/domain/src/tools/{BitcodeReadMeasurementComputerUseTool.ts,index.ts},packages/api/src/routes/shippables.ts,apps/uapi/app/executions/components/{ExecutionsPageClient.tsx,ExecutionsToggleButtons.tsx},apps/uapi/components/base/bitcode/execution/{toggle-buttons.tsx,execution-option-toggle.tsx}}`
  test coverage:
...
- `{packages/asset-packs-pipelines/domain/src/__tests__/metrics-output.test.ts,packages/prompts/{README.md,src/{index.ts,formatters/hierarchical.ts}},scripts/{generate-massive-prompt-parts.ts,normalize-asset-pack-promptparts.mjs},apps/uapi/tests/runner.agent.test.ts}`
  test coverage:
...
- `{packages/asset-packs-pipelines/domain/src/agents/prompts/asset-pack-comprehend-read-agent-prompts.ts,packages/prompts/src/{index.ts,index.d.ts,raw_promptparts/specific/{index.ts,promptpart_specific_agent_assetpacksetupcomprehendread_*}},protocol-demonstration/test/v26-shippable-reform.test.js,protocol-demonstration/V26_SHIPPABLE_REFORM.md}`
  test coverage:
  `protocol-demonstration/test/v26-shippable-reform.test.js` and `pnpm -C packages/pipelines/asset-pack typecheck`
  proof surface:
  fifth-gate prompt-reservoir recut closure for the active AssetPack setup/comprehend-read overlay so live prompt code imports `assetpacksetupcomprehendread` raw PromptParts, the old setup-comprehension file family is removed instead of aliased, superseded `deliverablesetup{comprehendtask,analyzecodebase,familiarizeattachments,preparerepository}` raw PromptPart families and root/barrel exports are removed after semantic mirror closure, the new attachment-evidence refinement PromptPart has TypeScript and runtime JavaScript parity, and the reform note now treats old setup-comprehension PromptPart names as removal targets rather than active module paths
- `{packages/asset-packs-pipelines/domain/src/agents/prompts/{clone-vcs-repository-prompt.ts,asset-pack-vcs-clone-repository-agent-prompts.ts},packages/prompts/src/{index.ts,index.d.ts,raw_promptparts/specific/{index.ts,promptpart_specific_agent_assetpacksetupclonerepository_*,promptpart_specific_agent_assetpackpipeline_clonevcsrepository_*}},protocol-demonstration/test/v26-shippable-reform.test.js,protocol-demonstration/V26_SHIPPABLE_REFORM.md}`
  test coverage:
  `protocol-demonstration/test/v26-shippable-reform.test.js` and `pnpm -C packages/pipelines/asset-pack typecheck`
  proof surface:
