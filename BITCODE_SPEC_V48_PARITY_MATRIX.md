...
| Terminal eradication | No live product imports from `uapi/app/terminal/`; `/terminal` redirect-only or removed; nav/login not Terminal CTAs | pending | product-routes + repository-context relocated to Bitcode; deposits/reads/auxillaries partially decoupled; remaining live Terminal modules and cockpit delete continue | version/v48 workstream |
