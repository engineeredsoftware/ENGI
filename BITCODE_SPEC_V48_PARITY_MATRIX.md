...
| Terminal eradication | No live product imports from `uapi/app/terminal/`; `/terminal` redirect-only or removed; nav/login not Terminal CTAs | pending | product-routes, repository-context, run-data/activity/transactions, readiness, pipelines table, Auxillaries open button relocated; remaining: read workbench panels (DepositReadWorkbench, RepositoryContextPanel, ReadScenarioPanel) | version/v48 workstream |
