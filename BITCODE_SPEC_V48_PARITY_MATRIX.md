...
| Frontend component architecture | Three layers (`Shadcn*` → `Bitcode*` → seven experiences) under `uapi/components/{shadcn,bitcode,marketing,packs,reads,deposits,docs,conversations,auxillaries}` with thin `uapi/app` page shells | pending | SPEC § Frontend component and naming architecture; NOTES workstream; scaffold READMEs land first; tree move of `components/base/*` continues in Phase 1 | version/v48 workstream |
| Pipeline naming (product UI) | Product run surfaces use Pipeline names (`BitcodePipeline*`, experience extensions); Execution UI names retired; agent `execution-generics` retained as non-product primitive | pending | Law specified; shared `bitcode/execution` rename and executions corridor follow Phases 2 and 6 | version/v48 workstream |
| Terminal eradication | No live product imports from `uapi/app/terminal/`; `/terminal` redirect-only or removed; nav/login not Terminal CTAs | pending | ~15 live modules still imported by deposits/reads; ~81 cockpit-only candidates for delete after relocate; product-routes extracted to Bitcode | version/v48 workstream |
