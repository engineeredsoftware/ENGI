...
| Frontend component architecture | Three layers (`Shadcn*` → `Bitcode*` → seven experiences) under `uapi/components/{shadcn,bitcode,marketing,packs,reads,deposits,docs,conversations,auxillaries}` with thin `uapi/app` page shells | substantially advanced | Phase 1 moved shadcn/bitcode out of `components/base/`; experience dirs scaffolded; `Shadcn*` export renames and experience component relocation remain | version/v48 workstream |
